...
class experience_buffer():
    ...
    def sample(self,batch_size,trace_length):
        ...
        for episode in sampled_episodes:
            ep = list(episode)
            #print(ep)
            #print(len(ep)+1-trace_length)
            point = np.random.randint(0,len(ep)+1-trace_length)
            sampledTraces.append(ep[point:point+trace_length])
            episode = zip(ep)
...
with tf.Session() as sess:
    ...
    for i in range(num_episodes):
        ...
        while j < max_epLength: 
            ...
            #if(len(np.reshape(np.array([s,a,r,s1,d]),[1,5])) == 0):
            #if (np.reshape(np.array([s,a,r,s1,d]),[1,5]) == np.array([])):
            #	print("AHHHH")
        ...
        episodeBuffer = zip(bufferArray) #was zip(bufferArray)
        #print(len(list(episodeBuffer)))
        myBuffer.add(list(episodeBuffer))
        jList.append(j)
        rList.append(rAll)
        print(i)
        ...
        if len(rList) % summaryLength == 0 and len(rList) != 0:
            ...
            epBuff = list(episodeBuffer)
            #saveToCenter(i,rList,jList,np.reshape(np.array(epBuff),[len(epBuff),5]),\
