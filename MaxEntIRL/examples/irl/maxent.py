...
import irl.value_iteration as value_iteration #from .value_iteration import *

def irl(feature_matrix, n_actions, discount, transition_probability,
        trajectories, epochs, learning_rate, debug=False):
    """
    ...
    for i in range(epochs):
        ...
        expected_svf,policy = find_expected_svf(n_states, r, n_actions, discount,
                                         transition_probability, trajectories)
        grad = feature_expectations - feature_matrix.T.dot(expected_svf)
        
        if debug:
            gradient_over_epochs.append(np.mean(grad))
            #print(gradient_over_epochs)
            ln.set_xdata(range(len(gradient_over_epochs)))
            ln.set_ydata(gradient_over_epochs)
            ax.relim()
    ...
    return feature_matrix.dot(alpha).reshape((n_states,)), gradient_over_epochs,policy
