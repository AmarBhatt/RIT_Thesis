...
class Taskworld(object):
    """
    Gridworld MDP.
    """

    def __init__(self, grid_size, wind,goals):
        """
        ...
        self.paths = {}
        for goal in len(self.goals):
            self.paths[str(goal)] = self.get_all_shortest_paths(self.goals[goal])
    ...
    def create_world(self):
        ...
        for goal in len(self.goals)
            (x,y) = self.int_to_point(self.goals[goal])
            w[y,x] = goal+1
        return w
