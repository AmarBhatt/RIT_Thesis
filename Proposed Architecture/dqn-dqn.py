...
with graph_dqn.as_default():
	...
	net = DQN(X,Y,n_classes,pooling_bool)
	dqn = net.outpost
	...
	# cost_1 = tf.Print(cost_1,[cost_1],message="cost_1: ", summarize=100)
...
with tf.Session(graph=graph_dqn) as sess:
	...
	for epoch in range(num_epochs):
		...
		s_prime = sess.run(dqn_presoft,feed_dict={X: s_prime, Y: a})
  ...
  #             "{:.5f}".format(acc))
	...
	for epoch in range(0,num_epochs_ran):
		...
		action = sess.run(dqn_presoft,feed_dict={X: [pixels]})
		...
		if(epoch%test_interval == 0):
			# Test Network
			win,lose,cur_path_total,total_path = test_network(sess,dqn_presoft, X, epoch,len(test_array), same, location,test_image_location,test_array, normalize, data_size, actual_size)
