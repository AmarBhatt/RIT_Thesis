...
%DIF ADD Thesis.tex                  Mon May 15 01:25:08 2017
%
% THESIS DESCRIPTION:
...
%DIF < | \fivesigstrue but don't define BOTH to be true!! 
\DIFdelbegin %DIFDELCMD < \foursigstrue
%DIFDELCMD < %%%
\DIFdelend %DIF > \fivesigstrue %but don't define BOTH to be true!! 
%DIF > \foursigstrue
\DIFaddbegin \fivesigstrue
\DIFaddend 
    ...
    \DIFdelbegin %DIFDELCMD < \thirdreader{Louis Beato}{Lecturer}%%%
\DIFdelend \DIFaddbegin \thirdreader{Dr. Christopher Kanan}{Assistant Professor}\DIFaddend %
    \DIFdelbegin %DIFDELCMD < \thirddept{Computer Engineering}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \thirddept{Imaging Science}
    \fourthreader{Louis Beato}{Lecturer}%DIF > 
    \fourthdept{Computer Engineering}
\DIFaddend %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Use this only if \foursigstrue
...
\vfill
\begin{center}
\DIFdelbegin \DIFdel{To whoever}\DIFdelend \DIFaddbegin \DIFadd{This thesis is dedicated to all those who have taught and guided me throughout my years}\DIFaddend ... 
\DIFaddbegin 


\DIFadd{My loving parents who continue to support me in my academic and personal endeavors.  While the strict upbringing could have been a bit more lenient, their emphasis on respect, diligence, and never giving up crafted me as a student and a person.  
}


\DIFadd{My teachers, professors, coaches, gurus, and senseis whose commitment to excellence, student development, and higher education brought light to what they taught me.  Thank you for being my guide and imparting your knowledge on me.
}


\DIFadd{My faith for giving me strength in tough times, and giving me the ability to help those around me.
...
\DIFadd{For my advisor, Dr. Ptucha who brought me under his wing as a young undergraduate.  His guidance, mentorship, and ability to unlock true potential shaped me as a student and an academic.  I thank him for trusting me with Milpet, and being there for me in my journey through higher education.  
}


\DIFadd{For my thesis committee which is filled with a number of professors from different fields.  Their diverse knowledge represents my passion for multidisciplinary research.  They each have contributed to my academics in unique ways, and for that I am grateful.
}


\DIFadd{For my friends for staying up with me late nights and collaborating on projects.  To Luke Boudreau thank you for being a great friend.  To Felipe Petroski Such, you are one of the most intelligent people I know, thank you for all of your insight. To Radha Mendapra for continually supporting me through my highs and my lows.
}


\DIFadd{For my parents who funded my pursuit into higher education and whose unwavering support gave me the strength to continue forward.
}


\DIFadd{For RIT, for giving me the opportunity to succeed both as an academic and as a student leader.  This University's commitment to student excellence is incredible, and rare.
}
...
\DIFdelbegin \subsubsection{\DIFdel{Q-Learning Algorithm}}
%DIFAUXCMD
\addtocounter{subsubsection}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \paragraph{\DIFadd{Q-Learning Algorithm}}
\DIFaddend To implement the Q-learning algorithm, the high-level steps outlined in \DIFdelbegin \DIFdel{Fig. \ref{fig:q-learning} }\DIFdelend \DIFaddbegin \DIFadd{Algorithm \ref{alg:q-learning} }\DIFaddend were used.  As shown by the algorithm, a random state is chosen from the generated Q-matrix used to hold the values to determine the best path.  Then an action is taken in the current state based on the \begin{math}\epsilon\end{math}-greedy selection policy.  Using this action, the next state transition is determined as well as the reward for taking the action.  These values then update the Q-matrix at the current state as described by (\ref{eq:Q-learning value function}). This is then repeated with the next state, until the current state is the goal. As shown in the algorithm, the Q-matrix at each state is updated based on a learning parameter, \begin{math}\alpha\end{math} which is set to 1 in this application to assure quick learning. This matrix also depends on a discount factor denoted by \begin{math}\gamma\end{math} which is set to 0.8 to give importance to future rewards~\cite{Eden}. This was repeated for 10,000 episodes where at the start of each episode a random starting state was chosen. 
\DIFdelbegin %DIFDELCMD < \begin{figure}
%DIFDELCMD < \centering
%DIFDELCMD < \begin{lstlisting}[frame=single]
%DIFDELCMD < %%%
\DIFdelFL{Choose a Q(s,a)
For each episode
    Set state s
    while s is not goal
        Choose action a from s }\DIFdelendFL \DIFaddbeginFL 

\begin{algorithm}
\caption{\DIFaddFL{Q-Learning Algorithm~\mbox{%DIFAUXCMD
\cite{Eden}}%DIFAUXCMD
}}
\label{alg:q-learning}
\begin{algorithmic} 
\For{e $\in$ number of episodes}
\State \DIFaddFL{Pick random state $s$
}\While {\DIFaddFL{$s$ is not the goal}}
\State \DIFaddFL{Choose action $a$ from $s$ }\DIFaddendFL using epsilon policy from Q
\DIFdelbeginFL \DIFdelFL{Take action a}\DIFdelendFL \DIFaddbeginFL \State \DIFaddFL{Take action $a$}\DIFaddendFL , observe reward \DIFdelbeginFL \DIFdelFL{r 
            }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$r$ }\DIFaddendFL and next state \DIFdelbeginFL \DIFdelFL{s'
        Update Q-value as followsQ(s,a) <- Q(s,a) + $\alpha$ }%DIFDELCMD < [%%%
\DIFdelFL{r+ $\gamma$ $max_{\alpha}$(
            Q(s',a')) - Q(s,a)}%DIFDELCMD < ] %%%
\DIFdelFL{(*@}%DIFDELCMD < \newline%%%
\DIFdelFL{@*)
        set s to s'
...
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Q-Learning Algorithm~\mbox{%DIFAUXCMD
\cite{Eden}}%DIFAUXCMD
}}
%DIFAUXCMD
%DIFDELCMD < \label{fig:q-learning}
%DIFDELCMD < \end{figure}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{$s'$
}\State \DIFadd{Update Q-values as follows:
}\State \DIFadd{$Q(s,a) \leftarrow Q(s,a) + \alpha [r+ \gamma*max_{\alpha}(Q(s',a')) - Q(s,a)] $
...
For each episode
    Set state s
    Choose action a from s }\DIFdelendFL \DIFaddbeginFL \begin{algorithm}
\caption{\DIFaddFL{Sarsa Algorithm~\mbox{%DIFAUXCMD
\cite{Eden}}%DIFAUXCMD
}}
\label{alg:sarsa}
\begin{algorithmic} 
...
\DIFdelbeginFL \DIFdelFL{while s is not goal
        Take action a}\DIFdelendFL \DIFaddbeginFL \While {\DIFaddFL{$s$ is not the goal}}
\State \DIFaddFL{Take action $a$}\DIFaddendFL , observe reward \DIFdelbeginFL \DIFdelFL{r 
            }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$r$ }\DIFaddendFL and next state \DIFdelbeginFL \DIFdelFL{s'
        Choose action a' from s' }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$s'$
}\State \DIFaddFL{Choose action $a'$ from $s'$ }\DIFaddendFL using epsilon policy from Q
\DIFdelbeginFL \DIFdelFL{Update Q-value as followsQ(s,a) <- Q(s,a) + $\alpha$ }%DIFDELCMD < [%%%
\DIFdelFL{r+$\gamma$ Q(s',a') 
            - Q(s,a)}%DIFDELCMD < ]
%DIFDELCMD <         %%%
\DIFdelFL{set s to s'
        set a to a'
...
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Sarsa Algorithm~\mbox{%DIFAUXCMD
\cite{Eden}}%DIFAUXCMD
}}
%DIFAUXCMD
%DIFDELCMD < \label{fig:sarsa}
%DIFDELCMD < \end{figure}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \State \DIFadd{Update Q-values as follows:
}\State \DIFadd{$Q(s,a) \leftarrow Q(s,a) + \alpha [r+ \gamma*max_{\alpha}(Q(s',a')) - Q(s,a)] $
}\State \DIFadd{$s \leftarrow s'$
}\State \DIFadd{$a \leftarrow a'$
}\EndWhile
\EndFor
\end{algorithmic}
\end{algorithm}
...
\begin{figure}
\centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics[scale=.5]{images/q-learning-example.PNG}
%DIFDELCMD < %%%
...
\DIFdelbeginFL %DIFDELCMD < \includegraphics[scale=.5]{images/sarsa-example.PNG}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[scale=.75]{images/sarsa-example.PNG}
\DIFaddendFL \caption{Sarsa Algorithm Path Example with Punishment}
...
\DIFdelbegin \chapter{\DIFdel{Apprenticeship Learning (Inverse Reinforcement Learning)}}
	%DIFAUXCMD
\addtocounter{chapter}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \section{\DIFadd{Apprenticeship Learning (Inverse Reinforcement Learning)}}
...
%DIFAUXCMD
\addtocounter{section}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \subsection{\DIFadd{Bayesian Inverse Reinforcement Learning}}
...
\DIFadd{In terms of the expert, the goal is to maximize the accumulated reward which is equivalent to performing the action which causes a states }\textit{\DIFadd{$Q^*$}} \DIFadd{value to be maximum. Using this, the more confident we are in the expert's ability to choose a good action (denoted as $\alpha_X$) the likelihood increases on the prediction that the expert would choose action }\textit{\DIFadd{a}} \DIFadd{in state }\textit{\DIFadd{s}}\DIFadd{.  This is shown in (\ref{eq:bayseianliklihood}).
}
...
\label{sec:gpirl}
\DIFadd{An issue with many inverse reinforcement learning algorithms is that it assumes a linear reward function.  This limits the extent of what inverse reinforcement learning can learn from an expert.  For example, in a highway driving model used in~\mbox{%DIFAUXCMD
...
%DIFAUXCMD
\addtocounter{section}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \subsection{\DIFadd{IRL using DQN}}
...
\DIFaddend Mnih et al.\DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
...
\begin{figure}
\centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics[scale=.85]{images/DQN-atari.png}
%DIFDELCMD < %%%
...
%DIFDELCMD < %%%
\chapter{\DIFdel{Platform}}
%DIFAUXCMD
\addtocounter{chapter}{-1}%DIFAUXCMD
\DIFdel{Generally environments that make use of automatic guided vehicles (AGVs) have to plan the path(s) where the robots should go before installing the tracks, like magnetic strips or metal tracks; this is an investment even before using the robots. If any change to the path(s ) is required to be made, then more cost is incurred. For this research a four wheeled differential drive robot has been controlled wirelessly to follow paths drawn on a graphical user interface within a workspace of 1.8m by 1.4m. The robot is controlled by correcting its orientation through visual feedback from a camera.  Error analysis was performed to investigate how well the robot followed the path drawn. The estimated error of }\DIFdelend \DIFaddbegin \subsection{\DIFadd{Deep Apprenticeship Learning (DAL)}}
\label{sec:DAL}
\DIFadd{Markovikj~\mbox{%DIFAUXCMD
\cite{markovikj2014deep} }%DIFAUXCMD
used a deep architecture to perform IRL on various games using raw pixel inputs. These games were Atari games; Freeway, Space Invaders, Seaquest. This was done through the creation of a multi-part architecture known as Deep Apprenticeship Learning. The two parts of this architecture are Deep Apprenticeship Q-Network (DAQN) and Deep Apprenticeship Reward Network (DARN).
}\subsubsection{\DIFadd{Deep Apprenticeship Q-Network (DAQN)}}
\DIFadd{The DAQN~\mbox{%DIFAUXCMD
\cite{markovikj2014deep} }%DIFAUXCMD
is used to take in raw inputs from expert game play ($D_E$) to learn a reward (or score) function which rates actions for states, much like }\DIFaddend the \DIFdelbegin \DIFdel{robot is within a few centimeters of the path and can be reduced by modifying various thresholds.
}\DIFdelend \DIFaddbegin \DIFadd{Q-function. Therefore, training this network will output the expert's policy. This network created using convolutional layers, similar to the Deep Q-Network~\mbox{%DIFAUXCMD
\cite{atari}}%DIFAUXCMD
. The inputs to this network are 83x83 raw pixel images.  Tests were done with feeding in 1-4 frames per input. The network was updated using batch processing of 32 samples. The structure is shown in Fig.~\ref{fig:daqn-dal}.
}\begin{figure}
\centering
\includegraphics[scale=1]{images/daqn-dal.png}
\caption{\DIFaddFL{DAQN Architecture~\mbox{%DIFAUXCMD
\cite{markovikj2014deep}}%DIFAUXCMD
.}}
\label{fig:daqn-dal}
\end{figure}
\DIFadd{As shown, this architecture outputs a softmax prediction between three possible actions (stay, up, down).  Therefore, for every state (raw pixel input) the network predicts the next action to be taken. The loss function of this network is shown in~\ref{eq:daqn-loss}.
}\begin{equation}
     \DIFadd{\label{eq:daqn-loss}
     \resizebox{0.5\textwidth}{!}{$J(w) =\sum_{a}[q_w(s,a)-\hat{q}(s,a)]^2$}~\mbox{%DIFAUXCMD
\cite{markovikj2014deep}}%DIFAUXCMD
%DIF >  
}\end{equation}
\DIFadd{Where, $w$ are the learned weights, $q_w(s,a)$ is the output of the DAQN and $\hat{q}(s,a)$ is the actual action taken by the expert represented by a 1-hot array.  Therefore, for inputs ($s=s_n,a=a_n \in D_E$), the array $\hat{q}$ is 1 if $a = a_n$ and 0 otherwise. The network is updated using AdaGrad where each parameter has its own learning rate, $\eta$ learned over time.
}\DIFaddend 

\DIFdelbegin \section{\DIFdel{Background}}
%DIFAUXCMD
\addtocounter{section}{-1}%DIFAUXCMD
\DIFdel{Robots whose path is controlled by the user are termed as automatic guided vehicles (AGVs).  AGVs follow paths using lasers, cameras, or are physically attached to these paths. These mobile robots follow predefined routes and so, are heavily used in industries for materials handling. Apart from transporting loads, these kinds of robots are used in risky environments and explorations, like space, mines and underwater. For guiding some AGVs, lines, magnetic tracts, or wires are installed on warehouse floors.
Hence, making use of these robots requires an initial investment. Vision-based AGVs follow a physical line or markers in a room: landmark based navigation.  Combining these ideas a robotic system was created for dynamic path following using vision based control.  
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{This research platform uses a wireless automatic guided robot that does not require any kind of intrusive modifications to be made to the environment apart from installing an overhead camera. The camera above the workspace tracks the robot continuously and }\DIFdelend \DIFaddbegin \subsubsection{\DIFadd{Deep Apprenticeship Reward Network (DARN)}}
\DIFadd{Once the DAQN is trained, the DARN~\mbox{%DIFAUXCMD
\cite{markovikj2014deep} }%DIFAUXCMD
}\DIFaddend is used to \DIFdelbegin \DIFdel{help correct it to follow a predefined path.  The user is allowed to define the path the robot should follow in a GUI created in MATLAB.}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Using a live image of the environment from a bird's-eye-view, the robot is tracked in real-time using ArUco fiducial markers.  The live images are fed to a software GUI where }\DIFdelend \DIFaddbegin \DIFadd{extract the reward function from the learned expert policy in the DAQN.  The DARN has the same architecture as the DAQN (shown in Fig.~\ref{fig:daqn-dal}). However, it uses the output from }\DIFaddend the \DIFdelbegin \DIFdel{user can freehand draw any path they desire directly on top of the image. The system then sends commands to the robot to move it along this path by using the camera and ArUco markers as feedback control.
Doing this, }\DIFdelend \DIFaddbegin \DIFadd{DAQN before the softmax in its loss function.  The input to DARN is a $(s,a,s')$ pairing where $s$ is an 83x83 image, $a$ is the action taken at that state, and $s'$ is the next state after taking action $a$ from state $s$. The loss function for DARN uses an L2-norm and is shown in~\ref{eq:darn-loss}.
}\begin{equation}
     \DIFadd{\label{eq:darn-loss}
     \resizebox{0.5\textwidth}{!}{$J_r(w) =\|r_w(s,a) - \hat{r}(s,a) \|_2$}~\mbox{%DIFAUXCMD
\cite{markovikj2014deep}}%DIFAUXCMD
%DIF >  
}\end{equation}
\DIFadd{Where, $w$ are the learned weights, $r_w(s,a)$ is the output of the DARN and $\hat{r}(s,a)$ is a Bellman representation of the DAQN~\ref{eq:darn-bellman}.
}\begin{equation}
     \DIFadd{\label{eq:darn-bellman}
     \resizebox{0.7\textwidth}{!}{$\hat{r}(s,a) = DAQN^{PRESOFT}(s,a) - \gamma \max_{a'}DAQN^{PRESOFT}(s',a')$}~\mbox{%DIFAUXCMD
\cite{markovikj2014deep}}%DIFAUXCMD
%DIF >  
}\end{equation}
\DIFadd{Where, $DAQN^{PRESOFT}(s,a)$ are the presoft values of }\DIFaddend the \DIFdelbegin \DIFdel{user can draw complicated paths around a room and around obstacles for the robot to navigate.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\section{\DIFdel{System Overview}}
%DIFAUXCMD
\addtocounter{section}{-1}%DIFAUXCMD
\DIFdel{This system consisted of four main sub-components; the Robot, OpenCV, MATLAB, and the Environment workspace itself. Fig.~\ref{fig:workflow} shows the entire system architecture and work flow using these four main components. }%DIFDELCMD < \begin{figure}[h!]
%DIFDELCMD < \centering
%DIFDELCMD < \includegraphics[scale=.5]{images/workflow.PNG}
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{System Work Flow}}
%DIFAUXCMD
...
%DIFDELCMD < %%%
\subsection{\DIFdel{Differential Wheel Drive Robot}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
\DIFdel{Four-wheeled differential drive robotics platforms are commonly used innavigation applications.  The system used in this setup was the 4WD Rover I from Lynxmotion, as }\DIFdelend \DIFaddbegin \DIFadd{DAQN with inputs $(s,a)$ and $\max_{a'}DAQN^{PRESOFT}(s',a')$ is the maximum presoft value of the DAQN with inputs $(s',a')$. $\gamma$ is the discount factor on how much to weight future states. Therefore, the expanded loss function for the DAQN is shown in~\ref{eq:darn-loss-expanded}.  The overall training architecture for DARN is }\DIFaddend shown in Fig.~\DIFdelbegin \DIFdel{\ref{fig:robot}.
}%DIFDELCMD < 

%DIFDELCMD < \begin{figure}[h!]
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{\ref{fig:darn-dal}.
}\begin{equation}
     \DIFaddFL{\label{eq:darn-loss-expanded}
     \resizebox{0.7\textwidth}{!}{$J_r(w) =\|r_w(s,a) - (DAQN^{PRESOFT}(s,a) - \gamma \max_{a'}DAQN^{PRESOFT}(s',a'))\|_2$}~\mbox{%DIFAUXCMD
\cite{markovikj2014deep}}%DIFAUXCMD
%DIF >  
}\end{equation}
\begin{figure}
\DIFaddendFL \centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics[scale=.5]{images/robot.PNG}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[scale=1]{images/darn-dal.png}
\DIFaddendFL \caption{\DIFdelbeginFL \DIFdelFL{4WD Rover I from Lynxmotion}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{DAL Architecture~\mbox{%DIFAUXCMD
\cite{markovikj2014deep}}%DIFAUXCMD
.}\DIFaddendFL }
\DIFdelbeginFL %DIFDELCMD < \label{fig:robot}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \label{fig:darn-dal}
\DIFaddendFL \end{figure}
\DIFaddbegin \DIFadd{The DARN is trained using random state transitions contained in a separate dataset from the expert data, $D_g$.  This allows the system to generalize to all states, even ones not explored by the expert. 
}\DIFaddend 

\DIFdelbegin \DIFdel{In this type of system each pair of wheels can be controlled independently from another pair.
Therefore, this allows the robot to rotate about its center, which gives it greater control and speed on
sharp angled turns. 
To control these motors, pulse width modulation (PWM) signals are given to each motor. This type of signal is usually at a fixed frequency and is a digital square wave ranging from 0V to }\textit{\DIFdel{$V_{max}$}}%DIFAUXCMD
\DIFdel{. }\textit{\DIFdel{$V_{max}$}} %DIFAUXCMD
\DIFdel{for the specific application performed was 7.2V. The signal determines on and off time for the motor based on its duty cycle. 
A duty cycle of 50\% will result in the motors being powered at half their capacity.  A duty cycle of 10\% will result in the motors being powered at a tenth of their capacity.
A duty cycle of 100\% will result in the motors being powered at full capacity
...
would represent 100\% or full speed. }%DIFDELCMD < \begin{figure}[h!]
%DIFDELCMD < \centering
%DIFDELCMD < \includegraphics[scale=1]{images/PWM.PNG}
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Various duty cycles on PWM signals}}
%DIFAUXCMD
%DIFDELCMD < \label{fig:pwm}
%DIFDELCMD < \end{figure}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \subsubsection{\DIFadd{Results}}
\DIFadd{Once training on the DARN is complete, the system can now be used to predict next actions given a state. Due to the complexity of Space Invaders, this type of network did not generalize to its game play. However, it performed extremely well on the simple game of Freeway, obtaining a score much higher than other non-deep IRL algorithms~\mbox{%DIFAUXCMD
\cite{markovikj2014deep}}%DIFAUXCMD
. 
}\DIFaddend 

\DIFdelbegin \DIFdel{This robot was programmed through the Arduino to take in serial commands to determine motion.  Single character commands were used as shown in Table~\ref{tab:movement}.
To take in these serial commands, a Bluetooth module was connected to the Arduino's TX and RX lines.  The module used was the HC-06,which is alow-power slave Bluetooth module. Once the Arduino receives a movement command it sends the appropriate PWM signals to the Pololu Dual MC3926 motor controller board which was wired
directly to the motors, battery, and Arduino Uno.  }%DIFDELCMD < \begin{table}[h!]
%DIFDELCMD < \centering
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Movement Commands}}
%DIFAUXCMD
%DIFDELCMD < \label{tab:movement}
%DIFDELCMD < \begin{tabular}{|p{1.5cm}|p{1cm}|p{3cm}|}
%DIFDELCMD < \hline
%DIFDELCMD < %%%
\textbf{\DIFdelFL{Command}} %DIFAUXCMD
%DIFDELCMD < & %%%
\textbf{\DIFdelFL{Function}} %DIFAUXCMD
%DIFDELCMD < & %%%
\textbf{\DIFdelFL{Wheel Movement}} %DIFAUXCMD
%DIFDELCMD < \\ \hline
%DIFDELCMD < %%%
\DIFdelFL{F }%DIFDELCMD < & %%%
\DIFdelFL{Move Forward }%DIFDELCMD < & %%%
\DIFdelFL{All wheels turn in same direction, forward }%DIFDELCMD < \\ \hline
%DIFDELCMD < %%%
\DIFdelFL{L }%DIFDELCMD < & %%%
\DIFdelFL{Turn Left }%DIFDELCMD < & %%%
\DIFdelFL{Left side wheels reverse, right side wheels forward }%DIFDELCMD < \\ \hline
%DIFDELCMD < %%%
\DIFdelFL{R }%DIFDELCMD < & %%%
\DIFdelFL{Turn Right }%DIFDELCMD < & %%%
\DIFdelFL{Right side wheels forward, left side wheels reverse }%DIFDELCMD < \\ \hline
%DIFDELCMD < %%%
\DIFdelFL{S }%DIFDELCMD < & %%%
\DIFdelFL{Stop }%DIFDELCMD < & %%%
\DIFdelFL{Duty cycle of all wheels set to }\DIFdelendFL \DIFaddbeginFL \subsection{\DIFaddFL{Deep Q-Learning from Demonstrations (DQfD)}}
\DIFaddFL{Hester et al. undertook the challenge of bringing deep reinforcement learning to real world tasks~\mbox{%DIFAUXCMD
\cite{hester2017learning}}%DIFAUXCMD
.  They recognized, that while exploration in traditional reinforcement learning works well in simulation, it is not practical in physical environments.  Instead, they propose an algorithm that extends traditional DQNs to train with expert data using transfer learning theory.  They use important aspects of DQN such as Double DQN and experience replay.  To update the Q-values of the network with values from the target network the cost function in (\ref{eq:ddqn-dqfd}) is used.
}\begin{equation}
     \DIFaddFL{\label{eq:ddqn-dqfd}
     \resizebox{0.5\textwidth}{!}{$J_{DQ}(Q) =(R(s,a)+\gamma Q(s_{t+1},a^{max}_{t+1};\theta')-Q(s,a;\theta))^2$}~\mbox{%DIFAUXCMD
\cite{hester2017learning}}%DIFAUXCMD
%DIF >  
}\end{equation}
\DIFaddFL{Where $\theta'$ are the parameters of the target network, and $\theta$ are the parameters of the current network~\mbox{%DIFAUXCMD
\cite{van2016deep}}%DIFAUXCMD
.  However, due to the supervised learning step of using expert demonstrations, the cost function needs to be updated to include  (\ref{eq:ddqn-dqfd}).
}\begin{equation}
     \DIFaddFL{\label{eq:ddqn-dqfd}
     \resizebox{0.5\textwidth}{!}{$J_{E}(Q) =max_{a\in A}[Q(s,a)+l(s,a_E,a)]-Q(s,a_E)$}~\mbox{%DIFAUXCMD
\cite{hester2017learning}}%DIFAUXCMD
%DIF >  
}\end{equation}
\DIFaddFL{Where $a_E$ is the action the expert takes in state $s$.  $l(s,a_E,a)$ is a margin function where it is }\DIFaddendFL 0 \DIFdelbeginFL \DIFdelFL{\%, no movement }%DIFDELCMD < \\ \hline
%DIFDELCMD < \end{tabular}
%DIFDELCMD < \end{table}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{when $a=a_E$ and a positive number (usually 1) otherwise.  This will allow for the expert's decisions to be weighted higher.  An L2 regularization term is also used and applied to the weights and biases of the network to prevent from over-fitting on small expert datasets.  Therefore, the entire loss function is (\ref{eq:ddqn-dqfd}).
}\begin{equation}
     \DIFadd{\label{eq:ddqn-dqfd}
     \resizebox{0.5\textwidth}{!}{$J(Q) =J_{DQ}(Q) + \lambda_1J_E(Q) + \lambda_2J_{L2}(Q)$}~\mbox{%DIFAUXCMD
\cite{hester2017learning}}%DIFAUXCMD
%DIF >  
}\end{equation}
\DIFadd{Where $\lambda_1$ and $\lambda_2$ are weights for each of the losses. For experience replay, with probability $p$ an expert experience $(s,a,s',reward)$ will be chosen from the entire set of expert demonstrations and with probability $1-p$ a random demonstration will be chosen from the set of agent's experience held in a limited buffer.  When the buffer of experiences is full, older experiences will be written over with new ones.  The algorithm for DQfD is shown in (\ref{alg:dqfd}).
}\begin{algorithm}
\caption{\DIFadd{DQfD}}
\label{alg:dqfd}
\begin{algorithmic} 
\For{steps t $\in$ epochs}
\State \DIFadd{Get batch of $n$ transitions from expert data
}\State \DIFadd{Calculate loss J(Q) in (\ref{eq:ddqn-dqfd})
}\State \DIFadd{Update $\theta$ using gradient descent
}\EndFor
\For{steps t $\in$ epochs}
\State \DIFadd{Sample action from network given a state
}\State \DIFadd{Play action and observe (s',reward)
}\State \DIFadd{Store experience into replay buffer for agent experience (s,a,s',reward)
}\State \DIFadd{Get batch of $n$ transitions from expert experience ($p$) and agent experience ($1-p$)
}\State \DIFadd{Calculate loss J(Q) in (\ref{eq:ddqn-dqfd})
}\State \DIFadd{Update $\theta$ using gradient descent
}\State \DIFadd{At time interval $\tau$ update $\theta'$ with $\theta$
}\EndFor
\end{algorithmic}
\end{algorithm}
\DIFadd{Using the Atari game dataset and a custom-made game called Catch the DQfD algorithm was compared to DQN.  In most cases, DQfD learned faster due to the transfer learning step and out performed DQN and Double DQN.
}\DIFaddend 

\DIFdelbegin \subsection{\DIFdel{OpenCV}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{OpenCV is a comprehensive computer vision toolbox used for a variety of real-time applications such as tracking, facial recognition, segmentation, and classification.  It is originally written in C++ but wrappers have been made to allow it to be used in Python and MATLAB. For this system, the only component of OpenCV being used is video stream capture and the ArUco Marker detection.  ArUco markers are pattern-based black and white squares that can be easily identified through OpenCV.  Since OpenCV knows what patterns to look for, it can identify ArUco markers in an environment and determine the corners of them as well as the orientation.  For this application four ArUco markers were used as }\DIFdelend \DIFaddbegin \chapter{\DIFadd{Dataset}}
\section{\DIFadd{Maze World}}
\DIFadd{Maze World is a variation of Grid World, without pits.  An agent is placed inside a binary maze where white spaces are open and black spaces are walls the agent cannot go through.  The objective of the world is for the agent to navigate through the maze to a goal.  An example of this world is }\DIFaddend shown in Fig.~\DIFdelbegin \DIFdel{\ref{fig:ArUco}.
This marker array was printed to the size 14.5 cm by 14.5 cm and was  mounted flat on top of the robot so that the center of the array was at the center of the robot.  The top two markers were facing in the direction of the front of the robot and were used as a reference to determine the orientation of the robot.  }%DIFDELCMD < 

%DIFDELCMD < \begin{figure}[h!]
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{\ref{fig:maze_world_example}.
}\begin{figure}
\DIFaddendFL \centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics[scale=.45]{images/ArucoArray.PNG}
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{ArUco Array of four markers}}
%DIFAUXCMD
%DIFDELCMD < \label{fig:ArUco}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[scale=1.25]{images/maze_world_example.png}
\caption{\DIFaddFL{Maze World Example: White spaces are open, Black spaces are blocked, and the Gray space is the goal.}}
\label{fig:maze_world_example}
\DIFaddendFL \end{figure}
\DIFaddbegin \DIFadd{As shown in the figure, the world is composed of 100x100 pixels but only represents a 10x10 maze.  These mazes were generated in python using example code from~\mbox{%DIFAUXCMD
\cite{maze_code} }%DIFAUXCMD
which creates an $m x n$ grided maze in an $M x N$ pixel image, where $m$ and $n$ are factors of $M$ and $N$ respectively.  At any white space in the maze there exists a path to any other white space on the maze that is unobstructed by a black space.  This allows the goal to be put at any open white space in the maze. To make the dataset simpler, the mazes were converted into a single-channel 8-bit integer representation.  
}\DIFaddend 

\DIFdelbegin \subsection{\DIFdel{MATLAB}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{MATLAB is a powerful computing platform.  It uses a scripting based language which is ideal for fast prototyping.  It has also been built to be computationally efficient especially in terms of matrix math and neural networks.  In this system, MATLAB is used to create the main GUI, interface with OpenCV, interface with the camera, path creation and planning, and sending serial commands to the robot via Bluetooth.  
}%DIFDELCMD < 

%DIFDELCMD < %%%
\subsection{\DIFdel{Environment}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{The environment that the robot resides in can be variable.  However, the surface on which the robot roams needs to be navigable by the robot chassis. Also, the environment needs to be well lit so that the markers can be detected.  
A webcam was placed on the ceiling of the room which was about 8 feet from the ground and faced perpendicular to the floor.  The webcam used is }\DIFdelend \DIFaddbegin \DIFadd{An agent is placed in the maze randomly in any white space.  The agent is represented by a smaller dark gray square as }\DIFaddend shown in Fig.~\DIFdelbegin \DIFdel{\ref{fig:webcam}. The webcam itself was used with a resolution of 640 px by 480 px.  This created view-able workspace of about 1.8m (6 feet) by 1.4m (4.5 feet).  The robot was placed within this static workspace. The webcam was connected to the computer running the MATLAB application through USB. }%DIFDELCMD < 

%DIFDELCMD < \begin{figure}[h!]
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{\ref{fig:maze_with_agent}. The goal of the agent is to make it to the goal in the least number of steps.  The agent has four possible actions; Move Right, Move Down, Move Left, Move Up. Actions are one-hot enconded. If the agent hits a wall (black) it loses, if it hits the goal it wins. At each step, the agent moves a full grid step.  In the back-end, the maze is treated like a $m x n$ grid, and Python's PIL framework is used to generate the graphics used for simulation and dataset generation.
}\begin{figure}
\DIFaddendFL \centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics[scale=.15]{images/webcam.png}
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Webcam used for tracking}}
%DIFAUXCMD
%DIFDELCMD < \label{fig:webcam}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[scale=1.5]{images/maze_with_agent.jpg}
\caption{\DIFaddFL{Maze World with Agent: small, dark gray square represents agent.}}
\label{fig:maze_with_agent}
\DIFaddendFL \end{figure}

\DIFdelbegin \section{\DIFdel{Procedure}}
%DIFAUXCMD
\addtocounter{section}{-1}%DIFAUXCMD
\DIFdel{As an overview, the procedure of running this system takes into account all four main components.  First, the camera is set-up to view a bird's-eye view of the workspace.  The robot is then placed within that workspace with its ArUco markers. The application is then started using MATLAB. OpenCV functions are used to detect the ArUco markers and determine the millimeter to pixel ratio since the ArUco markers are a known size.
Once the robot is detected the user is prompted to draw a path over the current image. This path is then fed to the path planner code where the robot iteratively follows the following steps:
}%DIFDELCMD < \begin{itemize}
\begin{itemize}%DIFAUXCMD
%DIFDELCMD < \item %%%
\item%DIFAUXCMD
\DIFdel{The robot faces desired point
}%DIFDELCMD < \item %%%
\item%DIFAUXCMD
\DIFdel{The robot goes towards point
}%DIFDELCMD < \item %%%
\item%DIFAUXCMD
\DIFdel{Feedback from OpenCV to determine whether or not robot meets success thresholds
}%DIFDELCMD < \item %%%
\item%DIFAUXCMD
\DIFdel{The robot's movement is adjusted to meet threshold if needed
}%DIFDELCMD < \item %%%
\item%DIFAUXCMD
\DIFdel{Once the point is reached, the desired point moves to the next point
}
\end{itemize}%DIFAUXCMD
%DIFDELCMD < \end{itemize}
%DIFDELCMD < %%%
\DIFdel{Once the robot completes the desired path the errors from the robot movement versus the ground truth path are found.
The user is then prompted to draw another path. This basic procedure is shown in the flowchart in Fig~\ref{fig:flowchart}.  }\DIFdelend \DIFaddbegin \subsection{\DIFadd{Expert Data}}
\DIFadd{Expert data is needed to teach an agent through demonstration.  To generate expert data, a maze is created and an agent is placed randomly inside of the maze at any valid space. Then using Python's networkx library the 2D grided maze is converted into a graph.  Then using the networkx function $single\_source\_shortest\_path()$ every path from a valid point to the goal is computed using Dijkstra's algorithm. These paths are then used to allow the agent to traverse through the maze to the goal in the least amount of steps possible.
}\DIFaddend 

\DIFdelbegin %DIFDELCMD < \begin{figure}[h!]
%DIFDELCMD < \centering
%DIFDELCMD < \includegraphics[scale=.5]{images/flowchart.PNG}
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Basic Flowchart of Procedure}}
%DIFAUXCMD
%DIFDELCMD < \label{fig:flowchart}
%DIFDELCMD < \end{figure}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \subsection{\DIFadd{Random Data}}
...
\DIFdelbegin \subsection{\DIFdel{Detecting ArUco Markers}} 
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{To detect the 4 ArUco markers on the robot, the ArUco marker library within OpenCV was used. This library contains a function }\textit{\DIFdel{detectMarkers}} %DIFAUXCMD
\DIFdel{which takes in an image (that contains the markers) , a dictionary of marker definitions, and some optional parameters.  It then returns the corner locations and ID numbers for all markers detected.  Once the corners and IDs are found, as shown }\DIFdelend \DIFaddbegin \subsection{\DIFadd{Datasets}}
\DIFadd{Expert data and random data are stored as GIFs.  To do this, at each movement of the agent an image is generated (such as }\DIFaddend in Fig.~\DIFdelbegin \DIFdel{\ref{fig:ArUcodetected}, the center point for }\DIFdelend \DIFaddbegin \DIFadd{\ref{fig:maze_with_agent}) to represent the current state of the agent in the maze.  For expert data these images are collected until the agent reaches the goal. The images are then combined into a GIF and saved for later use.  Each action the agent takes is saved into a text file to be used later. The same is done for the random data, except that }\DIFaddend the \DIFdelbegin \DIFdel{array is also found. }%DIFDELCMD < \begin{figure}[h!]
%DIFDELCMD < \centering
%DIFDELCMD < \includegraphics[scale=.5]{images/ArucoDetected.PNG}
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Annotated ArUco Array of four markers}}
%DIFAUXCMD
%DIFDELCMD < \label{fig:ArUcodetected}
%DIFDELCMD < \end{figure}
%DIFDELCMD < %%%
\DIFdel{This is done by using a centroid function that uses the points of the 4 corners closest to the center of the image.  This is shown in Equation~\ref{eqn:centerx} and ~\ref{eqn:centery} where n is equal to 4 to represent the four points used.  This center point }\DIFdelend \DIFaddbegin \DIFadd{GIFs were only two images total.
}

\DIFadd{Two types of datasets were created for expert data and random data; same and random.
}\subsubsection{\DIFadd{Same}}
\DIFadd{"Same" means that the maze the agent is in does not change.  Therefore, the expert data and random data use the same maze, however the placement of the agent will change.  This dataset }\DIFaddend is used to \DIFdelbegin \DIFdel{determine the center of the robot, which isalso its turning axis.
}\begin{displaymath}
\DIFdel{center_x = \frac{\sum_{k=0}^{n}{x_k}}{n}
%DIFDELCMD < \label{eqn:centerx}%%%
}\end{displaymath}
%DIFAUXCMD
\begin{displaymath}
\DIFdel{center_y = \frac{\sum_{k=0}^{n}{y_k}}{n}
%DIFDELCMD < \label{eqn:centery}%%%
}\end{displaymath}
%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{show whether or not a system can learn a single task without generalizing what the task is.
}\DIFaddend 

\DIFdelbegin \subsection{\DIFdel{Drawing a Path}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \subsubsection{\DIFadd{Random}}
\DIFadd{"Random" means that the maze the agent is in does change.  Therefore, the expert data and random data use different randomly generated mazes, and the placement of the agent changes in each maze.  This dataset is used to show whether or not a system can learn a single task while also generalizing what the task is.
}\DIFaddend 

\DIFdelbegin \DIFdel{Once the robot and the markers are detected in }\DIFdelend \DIFaddbegin \subsection{\DIFadd{Simulation}}
\DIFadd{Given a maze, current placement of the agent, and the action to be performed, a simulation of the maze task can be completed.  This is used in testing as well as in training to show how well the agent is learning and to gather data.  At each step the simulation environment tells the system the new state of the agent (image of maze with agent), whether or not the agent has reached the goal, and whether or not the agent has crashed into a wall.  If the simulation is fed an initial maze image, it will keep that maze for the rest of the simulation (in cases where the same maze is being tested) otherwise it will use a random generated maze.  If the simulation is given no initial state, it will randomly place the agent in }\DIFaddend the \DIFdelbegin \DIFdel{frame of the workspace the user is prompted to draw a path.  This is done by using the built in }\textit{\DIFdel{imfreehand}} %DIFAUXCMD
\DIFdel{function on the current image of the workspace.  This allows the user to freely drawing any path within the workspace as shown in Fig.  ~\ref{fig:pathdrawn}.
}%DIFDELCMD < \begin{figure}[h!]
%DIFDELCMD < \centering
%DIFDELCMD < \includegraphics[scale=.5]{images/pathDrawn.PNG}
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Path Drawn by user in GUI}}
%DIFAUXCMD
%DIFDELCMD < \label{fig:pathdrawn}
%DIFDELCMD < \end{figure}
%DIFDELCMD <  %%%
\DIFdel{Once the user draws the path the path is represented by a solid red line with a white star at its first point and a yellow star at its end point.  }\DIFdelend \DIFaddbegin \DIFadd{maze.
}\DIFaddend 

\DIFdelbegin \subsection{\DIFdel{Accuracy Thresholds}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \subsection{\DIFadd{Processing Data}}
\DIFadd{To make the saved GIFs and action-list text files usable to the system architectures, some preprocessing needs to be done.  First, the GIFs are read in frame by frame.  These frames are then converted to the data size needed using PIL image resize function.  These are then saved into a numpy array of shape $[?,data size, data size, 1]$.  As shown, all the data is square.  The actions are read out of the action-list and converted into one-hot encoded vectors stored into a numpy array of shape $[?,number of actions]$.  These arrays are then split into test, train, and reward datasets for the image data and action data.  These split arrays are then stored into an h5py file so that it can be accessed faster and more efficiently when the system is run again. 
}\DIFaddend 

\DIFdelbegin \DIFdel{To ensure the robot follows the path accurately, thresholds were created for the allowable distance it was from the desired point and its orientation with respect to the next point.  To identify the distance in pixels between the desired point and the robot, denoted as }\textit{\DIFdel{d}}%DIFAUXCMD
\DIFdel{, the euclidean distance between the desired point and the center point of the ArUco marker array was found.  This equation is shown in \ref{eqn:distance}, where $x_c$ and $y_c$ are the pixel coordinates for the center of the robot, and $x_g$ and $y_g$ are the pixel coordinates for the desired point (current point on path trying to be reached) . If the value of }\textit{\DIFdel{d}} %DIFAUXCMD
\DIFdel{was less than the threshold (by default was 20 pixels) then the robot met the distance criteria.  }\DIFdelend %DIF > \chapter{Technologies and Implementations}
%DIF > \section{TensorFlow}

\DIFdelbegin \begin{displaymath}
\DIFdel{d = \sqrt[]{\DIFdel{(x_c - x_g)^2 + (y_c - y_g)^2}}}
\DIFdel{%DIFDELCMD < \label{eqn:distance}%%%
}\end{displaymath}
%DIFAUXCMD
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{The second criteria was the orientation of the robot to the next point}\DIFdelend \DIFaddbegin \chapter{\DIFadd{Proposed Architectures}}
\section{\DIFadd{Deep Apprenticeship Learning (DAL) Architecture Modification}}
\subsection{\DIFadd{No Pooling Layers}}
\DIFadd{One issue with the Deep Apprenticeship Learning (DAL) architecture in~\ref{sec:DAL}, is its use of pooling layers.  Pooling does well in classification to increase performance, however it removes many fine details that are necessary in task completion and understanding.  Removing the pooling after each convolution layer results in slower training, but allows the network to learn from smaller details within the images given.  For example, the using Maze World, pooling layers would remove the goal and the agent from existing after the first layer, causing all of the states going through the network to look almost identical}\DIFaddend .  This was \DIFdelbegin \DIFdel{important to meet so that the robot was always following the trajectory of the path.  To do this 3 vectors were found and plotted.  The first vector was from the center of the robot to }\DIFdelend \DIFaddbegin \DIFadd{shown in testing the DAL network when it would predict the same action be taken for each state presented.
}\subsection{\DIFadd{Transfer Learning}}
\DIFadd{Another change to }\DIFaddend the \DIFdelbegin \DIFdel{desired point ($V_g$).  The second vector was from the center of the robot to the front of the robot between ArUco markers 31 and 203 ($V_o$).  The third vector was from the front of the robot to the desired point ($V_n$).  These vectors are shown in Fig.~\ref{fig:orientation}.
}%DIFDELCMD < 

%DIFDELCMD < \begin{figure}[h!]
%DIFDELCMD < \centering
%DIFDELCMD < \includegraphics[scale=1]{images/angle.PNG}
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Setting Robot Heading Direction: white star indicates desired point; green indicates current heading direction between center and front of robot ($V_o$); blue indicates desired heading direction between center and desired point ($V_g$); magenta indicates difference between the directions which is the vector from the front of the robot to the desired point ($V_n$)}}
%DIFAUXCMD
%DIFDELCMD < \label{fig:orientation}
%DIFDELCMD < \end{figure}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{To use these vectors to modify the robot's current heading, they were first normalized by dividing the vector elements ($x$ and $y$) by the magnitude of the vector.  The dot product was then taken between $V_o$ and $V_g$ }\DIFdelend \DIFaddbegin \DIFadd{DAL architecture is the use of transfer learning.  In the current implementation of DAL, the DAQN and DARN are trained seperatley.  This means that, while they use the same network architecture, the DARN network starts with random weights and biases to learn from the DAQN network.  Instead, using a methodology called transfer learning, the DARN network is initialized with the same weights as the DAQN network before being trained.  This allows the DARN network to have a good starting point in understanding the structure of the states so that its main focus is on updating its Q-values and not processing the image.  This also allows it to learn faster, and with a lower cost.
}\subsection{\DIFadd{Using Q-Learning}}
\DIFadd{On top of removing pooling layers and including transfer learning, methodologies from Q-learning were also implemented.  In its current form, the DAQN network in DAL does not use Q-updates to compute its loss.  Instead it uses the softmax of the output of the network to compare it to the action it is supposed to be.  While a method like this would work in classification, it is unstable in task learning because it leads to large jumps in updating the weights of the network.  Instead the loss function was changed to be more representative of Q-learning for both the DAQN and DARN networks }\DIFaddend as shown in\DIFdelbegin \DIFdel{Equation~\ref{eqn:orientation}. Where $a$ is a value between 1 and -1. The desired orientation value of the robot is 1 }\DIFdelend \DIFaddbegin \DIFadd{~\ref{eq:dal-dqn-loss}
}

\begin{equation}
     \DIFadd{\label{eq:dal-dqn-loss}
     \resizebox{0.5\textwidth}{!}{$J(Q) =(R(s,a)+\gamma*max_{a'}Q(s',a'))-Q(s,a))^2$} 
}\end{equation}
\subsubsection{\DIFadd{Reward Abstraction}}
\DIFadd{An inherit issue with this representation is the need for the reward $R$.  The purpose of apprenticeship learning is to find the reward model from given expert demonstrations, }\DIFaddend which means that the \DIFdelbegin \DIFdel{robot's heading is facing towards (parallel) the desired point.  At -1, }\DIFdelend \DIFaddbegin \DIFadd{reward for each step an agent or expert takes is not known. By abstracting the reward function from step-level to task-level, it serves as a task completion modifier for the loss.  For example, for each step taken by the expert if the task is not complete the reward is $0$, if the task is complete the reward is $+1$, and if the task is failed the reward is $-1$.  Since the expert data used is all successful, only the last transition for the expert into the goal is given a reward of $+1$.  For the DARN network, the same loss function is used, but instead of using random data (\ref{sec:random_data}) the network "plays against itself" to accrue more experience.  Therefore, the agent starts in a maze in a random location and performs a step based on the network output from the state it is in.  If the step is to }\DIFaddend the \DIFdelbegin \DIFdel{robot is facing the opposite way of the desired point. An orientation threshold was defined to be close to 1 to make the robot more accurate in path following.  }\begin{displaymath}
\DIFdel{a = V_o  \bm{\cdot} V_g
%DIFDELCMD < \label{eqn:orientation}%%%
}\end{displaymath}
%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{goal the reward is $+1$ if it hits a wall it is $-1$ (and the maze resets), otherwise it is $0$.  
}\DIFaddend 

\DIFdelbegin \DIFdel{The $a$ value gives us information on heading but does not tell the optimal way for the robot to turn.  The system could have been setup to turn left every time the heading was less than the threshold until the orientation threshold was met, but that would not be optimal.  To determine the direction the robot should turn to make the most optimal decision to meet the orientation threshold Equation~\ref{eqn:theta} is used.  
Where the two parameters of the }\textit{\DIFdel{atan2}} %DIFAUXCMD
\DIFdel{function are the }\textit{\DIFdel{sin($\theta$)}} %DIFAUXCMD
\DIFdel{and }\textit{\DIFdel{cos($\theta$)}}%DIFAUXCMD
\DIFdel{.  This function returns the four quadrant inverse tangent that will give a $\theta$ value between 180$\degree$ and -180$\degree$. 
This gives us the exact angle between the $V_g$ and $V_n$ vectors to determine which direction to turn.  Since this angle is in reference to the current heading of the robot, a positive $\theta$ value will mean that the robot needs to turn left, and a negative $\theta$ value will mean that the robot needs to turn right.  }\begin{displaymath}
\DIFdel{\theta = atan2(V_{g_{x}}*V_{n_{y}}-V_{g_{y}}*V_{n_{x}}, V_{g_{x}}*V_{n_{x}}+V_{g_{y}}*V_{n_{y}}) 
%DIFDELCMD < \label{eqn:theta}%%%
}\end{displaymath}
%DIFAUXCMD
\DIFdelend \DIFaddbegin \subsubsection{\DIFadd{Experience Replay}}
\DIFadd{Upon creating experiences using the DARN network, the network will be updated through min-batches of experiences in the form of $(s,a,s',r)$ once its replay buffer is full.  As the agent has more experiences the older experiences are overwritten by the newer ones.  This allows the network to learn only from the most recent experiences to weed out "bad habits" from earlier experiences. 
}\DIFaddend 

\DIFdelbegin \DIFdel{Therefore, using the values $d$, $a$, and $\theta$ the robot's movement is altered using Algorithm~\ref{alg:movement}.  To ensure that the robot has enough time to act before another command is sent to it the movement commands were only sent once a certain time threshold was met. Therefore, some movement commands were skipped to allow the robot's movement to }\DIFdelend \DIFaddbegin \section{\DIFadd{Deep Q-Networks Implementations}}
\DIFadd{Many success have been seen in the realm of Deep Q-Learning and its modifications.  While this does extremely well in reinforcement learning, its methodologies have not been used much in deep apprenticeship learning.  Two methodologies from this space include shared experience and dueling deep Q-networks.  Both of these network modifications use the Deep Q-Network architecture~\mbox{%DIFAUXCMD
\cite{atari} }%DIFAUXCMD
which is fully convolutional, and has been proven successful in task completion environments in the Atari world.
}\subsection{\DIFadd{Using Shared Experience}}
\DIFadd{Experience replay~\mbox{%DIFAUXCMD
\cite{atari} }%DIFAUXCMD
has been shown to lead to better learning in networks by using past experience.  However, up until now, in traditional reinforcement learning only the agent's experience was used with this method.  While this worked for algorithms that relied on exploration and exploitation, it did not always produce great results if the experiences were not successful. This may cause the network to hit a local minima where the optimal action selection will never be reached. In apprenticeship learning, experiences from an expert are given and are guaranteed to }\DIFaddend be \DIFdelbegin \DIFdel{smooth and to ensure that the serial buffer on the robot would not be overflown.  }%DIFDELCMD < \begin{algorithm}
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdel{Robot Next Move}}
%DIFAUXCMD
%DIFDELCMD < \label{alg:movement}
%DIFDELCMD < \begin{algorithmic} 
%DIFDELCMD < \If{$d < distanceThreshold \land a > orientationThreshold$}
%DIFDELCMD < \State %%%
\DIFdel{$desiredPoint \leftarrow nextPoint$
}%DIFDELCMD < \ElsIf{$a < orientationThreshold$}
%DIFDELCMD < \If{$\theta > 0$}
%DIFDELCMD < \State %%%
\DIFdel{$Turn Left$
}%DIFDELCMD < \Else
...
%DIFDELCMD < \ElsIf{$d > distanceThreshold$}
%DIFDELCMD < \State %%%
\DIFdel{$Move Forward$
}%DIFDELCMD < \EndIf
...
%DIFDELCMD < %%%
\subsection{\DIFdel{Following the Path}} 
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Using Algorithm~\ref{alg:movement}, the robot went to each point on the path drawn by the user.  As it did this , the center point of }\DIFdelend \DIFaddbegin \DIFadd{correct.  Using these experiences along with the agent's learned experiences would allow for better learning.  Hester et al.~\mbox{%DIFAUXCMD
\cite{hester2017learning} }%DIFAUXCMD
introduced this in their DQfD architecture, and showed that just using 10\% of expert data in each batch update of experiences gave way to better learning, and ensured that the agent was not going to keep relearning from bad experiences.  This also ensures that the network can get out of a local minima and continue learning despite the "bad" learned experiences.
}\subsection{\DIFadd{Target Q-Network}}
\DIFadd{Mnih et al.~\mbox{%DIFAUXCMD
\cite{atari} }%DIFAUXCMD
also introduced the concept of using a separate target Q-network.  This was due to the fact that the Q-network architecure is unstable along sequential updates.  By updating the weights of }\DIFaddend the \DIFdelbegin \DIFdel{robot was plotted every time a point was reached.  Using this data the , the overall error of the physical robot path was determined.
Fig.~\ref{fig:pathfollowing} shows a snapshot of a robot following a path.  }%DIFDELCMD < 

%DIFDELCMD < \begin{figure}[h!]
%DIFDELCMD < \centering
%DIFDELCMD < \includegraphics[scale=.5]{images/star.PNG}
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Robot following the Path: white star indicates next point to reach; green markings show robot's past positions, yellow star indicates goal}}
%DIFAUXCMD
%DIFDELCMD < \label{fig:pathfollowing}
%DIFDELCMD < \end{figure}
%DIFDELCMD < 

%DIFDELCMD < %%%
\subsection{\DIFdel{Accuracy Analysis}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
\DIFdel{To determine the overall accuracy of the robot, error between the physical path the robot took and the actual path was measured. This accuracy was improved by changing the distance and orientation thresholds for the system.  At the end of each path taken, the error magnitude for each point is displayed as well as the standard deviation of error, mean error, maximum error, minimum error, and estimated error.  The estimated error is defined as the error that will be found 95\% of the time or two standard deviations ($\sigma$) away from the mean ($\mu$), as shown in Equation~\ref{eqn:esterror}.  }%DIFDELCMD < 

%DIFDELCMD < %%%
\begin{displaymath}
\DIFdel{Est. Error = \mu + 2*\sigma 
%DIFDELCMD < \label{eqn:esterror}%%%
}\end{displaymath}
%DIFAUXCMD
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Three trials were tested with varying threshold values.  Fig.~\ref{fig:results1}, ~\ref{fig:results2}, and ~\ref{fig:results3} show several paths with their corresponding error charts.  }%DIFDELCMD < 

%DIFDELCMD < \begin{figure}[h!]
%DIFDELCMD < \centering
%DIFDELCMD < \includegraphics[scale=.25]{images/PATH-2095-1.PNG}
%DIFDELCMD < \includegraphics[scale=.25]{images/DATA-2095-1.PNG}
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Results for robot motion with distanceThreshold = 20px and orientationThreshold = 0.95}}
%DIFAUXCMD
%DIFDELCMD < \label{fig:results1}
%DIFDELCMD < \end{figure}
%DIFDELCMD < 

%DIFDELCMD < \begin{figure}[h!]
%DIFDELCMD < \centering
%DIFDELCMD < \includegraphics[scale=.25]{images/PATH-2099.PNG}
%DIFDELCMD < \includegraphics[scale=.25]{images/DATA-2099.PNG}
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Results for robot motion with distanceThreshold = 20px and orientationThreshold = 0.99}}
%DIFAUXCMD
%DIFDELCMD < \label{fig:results2}
%DIFDELCMD < \end{figure}
%DIFDELCMD < 

%DIFDELCMD < \begin{figure}[h!]
%DIFDELCMD < \centering
%DIFDELCMD < \includegraphics[scale=.25]{images/PATH-1599.PNG}
%DIFDELCMD < \includegraphics[scale=.25]{images/DATA-1599.PNG}
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Results for robot motion with distanceThreshold = 15px and orientationThreshold = 0.99}}
%DIFAUXCMD
%DIFDELCMD < \label{fig:results3}
%DIFDELCMD < \end{figure}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{As shown in these figures, the robot's path seems to follow the path drawn by the user. Table~\ref{tab:error} shows these results in terms of standard deviation of error, mean error, maximum error, minimum error, and estimated error. Each of these values is converted to centimeters from pixels by using the known size of the workspace and the ArUco marker array}\DIFdelend \DIFaddbegin \DIFadd{Q-network by small amounts every iteration, the network is more likely to diverge due to tightly coupled correlations.  A target Q-network is used outside of the main Q network.  The main Q-network is updated at each iteration using values from the target Q-network.  After $\tau$ iterations, the target Q-network's weights are updated with main Q-network's weights. 
}\subsection{\DIFadd{Using Dueling DQN}}
\DIFadd{Another modification made was using the Dueling Deep Q-Network architecture (\ref{sec:DuelingDQN})~\mbox{%DIFAUXCMD
\cite{wang2015dueling}}%DIFAUXCMD
.  By splitting the value and advantage functions from the main Q-network, allows for the separation of state estimation and action selection.  By decoupling these two, the network can more accurately predict the best action to take regardless of the state that was predicted. For the dataset used (Maze World), this allows for part of the network to determine the current maze and placement of the goal, and the other part of the network to determine the shortest point to that goal}\DIFaddend . 

\DIFdelbegin %DIFDELCMD < \begin{table}[h!]
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Error Analysis}}
%DIFAUXCMD
%DIFDELCMD < \label{tab:error}
%DIFDELCMD < \begin{tabular}{ | p{0.5cm} | p{0.5cm} | p{0.5cm} | p{0.75cm} | p{0.75cm} | p{0.75cm} | p{1cm} | }
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \chapter{\DIFaddFL{Results}}
\DIFaddendFL 

\DIFaddbeginFL \DIFaddFL{Table of Task Completion (Same)~\ref{tab:task-complete}}
%}\begin{table}[]
%\centering
%\caption{\DIFaddFL{Task Completion Results}}
%\label{tab:task-complete}
%\begin{tabular}{|l|l|l|}
%\DIFaddendFL \hline
%\DIFaddbeginFL \multicolumn{3}{|c|}{Task Completion Results} \\ \hline
%\DIFaddendFL \textit{\DIFdelbeginFL \DIFdelFL{$a^*$}\DIFdelendFL \DIFaddbeginFL \textbf{\DIFaddFL{Networks}}\DIFaddendFL } & \textit{\DIFdelbeginFL \DIFdelFL{$d^*$}\DIFdelendFL \DIFaddbeginFL \textbf{\DIFaddFL{Tests Completed}}\DIFaddendFL } & \DIFdelbeginFL \DIFdelFL{STD }\DIFdelendFL \DIFaddbeginFL \textit{\textbf{\DIFaddFL{Correct Action Prediction}}} \\ \hline
%\DIFaddFL{DAL }\DIFaddendFL &  \DIFdelbeginFL \DIFdelFL{Mean Error }\DIFdelendFL &  \DIFdelbeginFL \DIFdelFL{Max Error }\DIFdelendFL \DIFaddbeginFL \\ \hline
%\DIFaddFL{DAL with Transfer Learning (no pooling) }\DIFaddendFL &  \DIFdelbeginFL \DIFdelFL{Min Error }\DIFdelendFL &  \DIFdelbeginFL \DIFdelFL{Estimated Error }\DIFdelendFL \\ \hline
%\DIFdelbeginFL \DIFdelFL{0.95 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{DAL with Bayesian Implementation }\DIFaddendFL &  \DIFdelbeginFL \DIFdelFL{20 }\DIFdelendFL &  \DIFdelbeginFL \DIFdelFL{0.68 }\DIFdelendFL \DIFaddbeginFL \\ \hline
%\DIFaddFL{DQN-AL }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{4.82 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{2/10 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{5.56 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{18.4\% (23/125) }\\ \hline
%\DIFaddFL{Dueling DQN-AL }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{1.30 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{9/10 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{6.19 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{76.8\% (96/125) }\DIFaddendFL \\ \hline
%\DIFdelbeginFL \DIFdelFL{0.99 }\DIFdelendFL \DIFaddbeginFL \end{tabular}
%\end{table}

\DIFadd{Table of Task Understanding (Random)~\ref{tab:task-understanding}}
%}\begin{table}[]
%\centering
%\caption{\DIFaddFL{Task Understanding Results}}
%\label{tab:task-understanding}
%\begin{tabular}{|l|l|l|}
%\hline
%\multicolumn{3}{|c|}{Task Understanding Results} \\ \hline
%\textit{\textbf{\DIFaddFL{Networks}}} \DIFaddendFL & \DIFdelbeginFL \DIFdelFL{20 }\DIFdelendFL \DIFaddbeginFL \textit{\textbf{\DIFaddFL{Tests Completed}}} \DIFaddendFL & \DIFdelbeginFL \DIFdelFL{0.68 }\DIFdelendFL \DIFaddbeginFL \textit{\textbf{\DIFaddFL{Correct Action Prediction}}} \\ \hline
%\DIFaddFL{DAL }\DIFaddendFL &  \DIFdelbeginFL \DIFdelFL{4.76 }\DIFdelendFL &  \DIFdelbeginFL \DIFdelFL{5.55 }\DIFdelendFL \DIFaddbeginFL \\ \hline
%\DIFaddFL{DAL with Transfer Learning (no pooling) }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{2.83 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{2/10 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{6.11 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{11.6\% (11/95) }\DIFaddendFL \\ \hline
%\DIFdelbeginFL \DIFdelFL{0.99 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{DAL with Bayesian Implementation }\DIFaddendFL &  \DIFdelbeginFL \DIFdelFL{15 }\DIFdelendFL &  \DIFdelbeginFL \DIFdelFL{0.76 }\DIFdelendFL \DIFaddbeginFL \\ \hline
%\DIFaddFL{DQN-AL }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{3.13 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0/10 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{4.01 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{9.5\% (9/95) }\\ \hline
%\DIFaddFL{Dueling DQN-AL }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{1.00 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{1/10 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{4.65 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{13.7\% (13/95) }\DIFaddendFL \\ \hline
%\DIFdelbeginFL %DIFDELCMD < \hline    
%%DIFDELCMD < \multicolumn{7}{ |c| }{Error in cm, $d^*$ in px} \\
%%DIFDELCMD < 
%
%%DIFDELCMD < \multicolumn{7}{ |c| }{\textit{$a^*$}= orientation threshold, \textit{$d^*$} = distance threshold} \\ \hline
%%DIFDELCMD < %%%
%\DIFdelendFL \end{tabular}
%\end{table}

\DIFdelbegin \DIFdel{As shown, as the threshold for orientation increased the estimated error decreased.  However, the largest contributor to the decrease in estimated error was the decrease in the distance threshold. Upon performing further tests, it was found that if the distance threshold is decreased then the orientation threshold must be increased, otherwise the robot would reach each point on the path but it will be unable to accurately produce the path due to the over-correction it will need to do. The orientation threshold contributes to lowering the over-correction needed. It was also found that lowering the distance threshold too much would result in the robot not being able to reach a point.  This is because while the ArUco marker detectionis robust, it is not very stable and does shift by a few pixels causing errors in the system when the distance threshold is too small.  Another threshold needing to be modified was the time threshold.  This threshold was used to determine how often movement commands would be sent to the robot.  If it was too high the robot would overshoot points, if it was too low then the robot would get overloaded with commands. Therefore, a threshold of 50 ms was used after testing various values.
}\DIFdelend \DIFaddbegin \DIFadd{Include loss graphs
}\DIFaddend 

\DIFdelbegin \chapter{\DIFdel{Conclusions}}
  %DIFAUXCMD
\addtocounter{chapter}{-1}%DIFAUXCMD
\DIFdel{...
  }\DIFdelend \DIFaddbegin \DIFadd{Include example images in sequence
}

\DIFadd{Table of results
}

\chapter{\DIFadd{Conclusions and Future Work}}
...
\DIFdelbegin \chapter{\DIFdel{A Long Proof}}
  %DIFAUXCMD
\addtocounter{chapter}{-1}%DIFAUXCMD
\DIFdelend %DIF > \chapter{A Long Proof}
  ...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\end{document}