...
%DIF DEL back-up\051517-2\Thesis.tex   Mon May 15 21:52:21 2017
%DIF ADD Thesis.tex                    Sun May 21 16:41:16 2017
%
% THESIS DESCRIPTION:
...
\DIFaddbegin \DIFadd{Lample et al.~\mbox{%DIFAUXCMD
\cite{DBLP:journals/corr/LampleC16} }%DIFAUXCMD
made a modification to this network to ensure only accurate gradients were being propagated through the network. Using the replay memory in sequences at a set trace length, only half of the errors of the that sequence are propagated back through the network.  More specifically, the first half of the trace errors are not used.  This was shown to cut-down on non-accurate information from updating the network.  Often the first set of moves in a trace are random, where as the last moves in a trace are more prone to be going on the right track.  By using this method, it was shown that the Atari game Doom was played at a much better rate than traditional DRQN and Dueling DQN~\mbox{%DIFAUXCMD
\cite{DBLP:journals/corr/LampleC16}}%DIFAUXCMD
.  However, the size of the trace, number of sampled episodes, and percentage of trace used in updates are hyper-parameters that need to be adjusted for each application the DRQN is used to learn.
}
...
\DIFaddbegin \label{sec:arch_details}
\DIFaddend A number of architectures were created and tested as described in Chapter~\ref{sec:proposed_architectures}. The \DIFaddbegin \DIFadd{descriptions of each network tested are as follows:
}\begin{itemize}
	\item \textit{\DIFadd{DAL}}\DIFadd{~\mbox{%DIFAUXCMD
\cite{markovikj2014deep}}%DIFAUXCMD
: The Deep Apprenticeship Learning architecture without any modifications.
	}\item \textit{\DIFadd{DAL}}\DIFadd{~\mbox{%DIFAUXCMD
\cite{markovikj2014deep} }%DIFAUXCMD
(no pooling): The Deep Apprenticeship Learning architecture without any pooling layers.
    }\item \textit{\DIFadd{DAL with Transfer Learning}}\DIFadd{: The Deep Apprenticeship Learning architecture where the trained DAQN network weights are used to initialize the weights of the DARN.
    }\item \textit{\DIFadd{DAL with Bellman}}\DIFadd{: The Deep Apprenticeship Learning architecture with using Bellman Q-value updates for both the DAQN and DARN, as well as experience replay.
    }\item \textit{\DIFadd{DQN-AL}}\DIFadd{: Deep Q-Network for Apprenticeship Learning which uses the DQN architecture and shared experience replay.
    }\item \textit{\DIFadd{DQN-AL with Scheduled Shared Experience}}\DIFadd{: Same as DQN-AL except that the expert experience for shared experience replay starts at a high percentage and decays over time.
	}\item \textit{\DIFadd{Dueling DQN-AL}}\DIFadd{: Dueling Deep Q-Network for Apprenticeship Learning which uses the Dueling DQN architecture (with Target Q-Network) and shared experience replay.
    }\item \textit{\DIFadd{Dueling DQN-AL with Scheduled Shared Experience}}\DIFadd{: Same as Dueling DQN-AL except that the expert experience for shared experience replay starts at a high percentage and decays over time.
 }\item \textit{\DIFadd{DRQN-AL}}\DIFadd{: Deep Recurrent Q-Network for Apprenticeship Learning which uses the DRQN architecture with experience replay modifications explained in Section~\ref{sec:drqn-implementation}.
}\end{itemize}

\DIFadd{The }\DIFaddend hyper-parameters for each of these architectures were tuned to ensure best results for the networks.  These hyper-parameters include:
\begin{itemize}
  ...
  \item Shared Experience Decay Frequency (SEF): Rate \DIFaddbegin \DIFadd{(number of epochs) }\DIFaddend at which shared experience ratio would be updated
  \item Replay Buffer: Size of experience memory
\end{itemize}
\DIFaddbegin \DIFadd{For the DRQN-AL implementation extra parameters needed to be defined.  These include:
}\begin{itemize}
	\item \DIFadd{Maximum Episode Number: Maximum number of episodes from dataset to sample from.
	}\item \DIFadd{Maximum Trace Length: Maximum length of trace to extract from each episode.
}\end{itemize}
\DIFaddend 

For each network the discount factor ($\gamma$) was kept constant at 0.9.  The dataset split for task completion for all DAL\DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\cite{markovikj2014deep} }%DIFAUXCMD
}\DIFaddend networks was 800 training, 200 testing, and 1000 random with a data size of $83\times 83\times 1$.  \DIFaddbegin \DIFadd{These were trained with a batch size of 50. }\DIFaddend For DQN networks it was 1000 train (the testing and random came from self generated experiences) with a data size of $84\times 84\times 1$. \DIFaddbegin \DIFadd{These were trained with a batch size of 32. }\DIFaddend The dataset split for task understanding for all DAL networks was 8000 training, 2000 testing, and 10000 random. For DQN networks it was 10000 train (the testing and random came from self generated experiences).  \DIFaddbegin \DIFadd{Also, for Dueling DQN the target Q-network update frequency was set for 4 epochs for task completion and 10 epochs for task understanding. }\DIFaddend The hyper-parameters for task completion and task understanding for each network are shown in Tables~\ref{tab:arch_hyp_same} and~\ref{tab:arch_hyp_rand}.  \DIFaddbegin \DIFadd{For the DRQN implementation, the maximum episode number was set to 8 and the maximum trace length was set to 4 to make an overall count of 32 data inputs per epoch.
}\DIFaddend 
...
DAL\DIFaddbeginFL \DIFaddFL{~\mbox{%DIFAUXCMD
\cite{markovikj2014deep} }%DIFAUXCMD
}\DIFaddendFL & T & 50000,100000 & 0.01 & \DIFaddbeginFL \DIFaddFL{- }\DIFaddendFL & \DIFaddbeginFL \DIFaddFL{- }\DIFaddendFL & \DIFaddbeginFL \DIFaddFL{- }\DIFaddendFL \\ \hline
DAL\DIFaddbeginFL \DIFaddFL{~\mbox{%DIFAUXCMD
\cite{markovikj2014deep} }%DIFAUXCMD
}\DIFaddendFL (no pooling) & F & 50000,100000 & 0.01 & \DIFaddbeginFL \DIFaddFL{- }\DIFaddendFL & \DIFaddbeginFL \DIFaddFL{- }\DIFaddendFL & \DIFaddbeginFL \DIFaddFL{- }\DIFaddendFL \\ \hline
DAL with Transfer Learning & F & 10000,10000 & 0.01 & \DIFaddbeginFL \DIFaddFL{- }\DIFaddendFL & \DIFaddbeginFL \DIFaddFL{- }\DIFaddendFL &  \\ \hline
DAL with \DIFdelbeginFL \DIFdelFL{Bayesian }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Bellman }\DIFaddendFL & F & 10000,10000 & \DIFaddbeginFL \DIFaddFL{0.01 }\DIFaddendFL & \DIFaddbeginFL \DIFaddFL{- }\DIFaddendFL & \DIFaddbeginFL \DIFaddFL{- }\DIFaddendFL & \DIFaddbeginFL \DIFaddFL{- }\DIFaddendFL \\ \hline
DQN-AL & F & 10000,10000 & 0.001 & \DIFaddbeginFL \DIFaddFL{0.1 }\DIFaddendFL & \DIFaddbeginFL \DIFaddFL{- }\DIFaddendFL & \DIFaddbeginFL \DIFaddFL{- }\DIFaddendFL \\ \hline
\begin{tabular}[c]{@{}l@{}}DQN-AL with \\ Scheduled \DIFdelbeginFL \DIFdelFL{Experience Replay}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Shared Experience}\DIFaddendFL \end{tabular} & F & 10000,10000 & 0.001 & \DIFaddbeginFL \DIFaddFL{0.9 }\DIFaddendFL & \DIFaddbeginFL \DIFaddFL{0.05 }\DIFaddendFL & \DIFaddbeginFL \DIFaddFL{500  }\DIFaddendFL \\ \hline
Dueling DQN-AL & F & 2000,10000 & \DIFaddbeginFL \DIFaddFL{0.001 }& \DIFaddendFL 0.1 & \DIFaddbeginFL \DIFaddFL{- }\DIFaddendFL & \DIFdelbeginFL %DIFDELCMD < &  %%%
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{- }\DIFaddendFL \\ \hline
\begin{tabular}[c]{@{}l@{}}Dueling DQN-AL with \\ Scheduled \DIFdelbeginFL \DIFdelFL{Experience Replay}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Shared Experience}\DIFaddendFL \end{tabular} & F & 2000,10000 & \DIFdelbeginFL \DIFdelFL{0.1 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.001 }\DIFaddendFL & \DIFaddbeginFL \DIFaddFL{0.9 }\DIFaddendFL & \DIFaddbeginFL \DIFaddFL{0.05 }\DIFaddendFL & \DIFaddbeginFL \DIFaddFL{500  }\DIFaddendFL \\ \hline
\DIFaddbeginFL \DIFaddFL{DRQN-AL }& \DIFaddFL{F }& \DIFaddFL{10000,100000 }& \DIFaddFL{0.001 }& \DIFaddFL{0.25 }& \DIFaddFL{- }& \DIFaddFL{- }\\ \hline
...
DAL with Transfer Learning & F & 10000,10000 & 0.01 & \DIFaddbeginFL \DIFaddFL{- }\DIFaddendFL & \DIFaddbeginFL \DIFaddFL{- }\DIFaddendFL & \DIFaddbeginFL \DIFaddFL{- }\DIFaddendFL \\ \hline
DAL with \DIFdelbeginFL \DIFdelFL{Bayesian }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Bellman }\DIFaddendFL & F & 10000,10000 & \DIFaddbeginFL \DIFaddFL{0.01 }\DIFaddendFL & \DIFaddbeginFL \DIFaddFL{- }\DIFaddendFL & \DIFaddbeginFL \DIFaddFL{- }\DIFaddendFL & \DIFaddbeginFL \DIFaddFL{- }\DIFaddendFL \\ \hline
DQN-AL & F & 10000,10000 & 0.001 & \DIFaddbeginFL \DIFaddFL{0.1 }\DIFaddendFL & \DIFaddbeginFL \DIFaddFL{- }\DIFaddendFL & \DIFaddbeginFL \DIFaddFL{- }\DIFaddendFL \\ \hline
\begin{tabular}[c]{@{}l@{}}DQN-AL with \\ Scheduled \DIFdelbeginFL \DIFdelFL{Experience Replay}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Shared Experience}\DIFaddendFL \end{tabular} & F & 10000,10000 & 0.001 & \DIFaddbeginFL \DIFaddFL{0.05 }\DIFaddendFL & \DIFaddbeginFL \DIFaddFL{500 }\DIFaddendFL &  \\ \hline
Dueling DQN-AL & F & 10000,10000 & 0.1 & \DIFaddbeginFL \DIFaddFL{0.1 }\DIFaddendFL & \DIFaddbeginFL \DIFaddFL{- }\DIFaddendFL &  \DIFaddbeginFL \DIFaddFL{-}\DIFaddendFL \\ \hline
\begin{tabular}[c]{@{}l@{}}Dueling DQN-AL with \\ Scheduled \DIFdelbeginFL \DIFdelFL{Experience Replay}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Shared Experience}\DIFaddendFL \end{tabular} & F & 10000,10000 & 0.1 & \DIFaddbeginFL \DIFaddFL{0.9 }\DIFaddendFL & \DIFaddbeginFL \DIFaddFL{0.05 }\DIFaddendFL & \DIFaddbeginFL \DIFaddFL{500 }\DIFaddendFL \\ \hline
\DIFaddbeginFL \DIFaddFL{DRQN-AL }& \DIFaddFL{F }& \DIFaddFL{10000,100000 }& \DIFaddFL{0.001 }& \DIFaddFL{0.25 }& \DIFaddFL{- }& \DIFaddFL{- }\\ \hline
\DIFaddendFL \end{tabular}
\end{table}
...
\subcaptionbox{Test 10\label{fig:TC Test 10}}{\includegraphics[scale=1]{images/task_comp_9.jpg}}\DIFaddFL{\ 
}\end{tabular}
\caption{\DIFaddFL{Task Completion Fixed Tests (10) with Agent Starting Location.}}
...
DAL\DIFaddbeginFL \DIFaddFL{~\mbox{%DIFAUXCMD
\cite{markovikj2014deep} }%DIFAUXCMD
}\DIFaddendFL & 1/10 & 6.4\% (8/125) \\ \hline
DAL\DIFaddbeginFL \DIFaddFL{~\mbox{%DIFAUXCMD
\cite{markovikj2014deep} }%DIFAUXCMD
}\DIFaddendFL (no pooling) & \DIFaddbeginFL \DIFaddFL{0/10 }\DIFaddendFL & \DIFaddbeginFL \DIFaddFL{5.6\% (7/125) }\DIFaddendFL \\ \hline
DAL with Transfer Learning (no pooling) & \DIFaddbeginFL \DIFaddFL{2/10 }\DIFaddendFL & \DIFaddbeginFL \DIFaddFL{10.4\% (13/125) }\DIFaddendFL \\ \hline
DAL with \DIFdelbeginFL \DIFdelFL{Bayesian }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Bellman }\DIFaddendFL Implementation & \DIFaddbeginFL \DIFaddFL{2/10 }\DIFaddendFL & \DIFaddbeginFL \DIFaddFL{8.8\% (11/125) }\DIFaddendFL \\ \hline
DQN-AL & 2/10 & 18.4\% (23/125) \\ \hline
\DIFdelbeginFL \DIFdelFL{Dueling }\DIFdelendFL \DIFaddbeginFL \begin{tabular}[c]{@{}l@{}}\DIFaddendFL DQN-AL \DIFaddbeginFL \DIFaddFL{with }\\ \DIFaddFL{Scheduled Shared Experience}\end{tabular} \DIFaddendFL & \DIFdelbeginFL \DIFdelFL{9}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{5}\DIFaddendFL /10 & \DIFdelbeginFL \DIFdelFL{76.8\% (}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{24.8\% (31/125)  }\\ \hline
\DIFaddFL{Dueling DQN-AL }& \textbf{\DIFaddFL{9/10}} & \textbf{\DIFaddFL{76.8\%}} \DIFaddFL{(}\DIFaddendFL 96/125) \\ \hline
\DIFaddbeginFL \begin{tabular}[c]{@{}l@{}}\DIFaddendFL Dueling DQN-AL with \DIFdelbeginFL \DIFdelFL{Scheduled Experience Replay }\DIFdelendFL \DIFaddbeginFL \\ \DIFaddFL{Scheduled Shared Experience}\end{tabular} \DIFaddendFL & \DIFaddbeginFL \DIFaddFL{7/10 }\DIFaddendFL & \DIFaddbeginFL \DIFaddFL{58.4\% (73/125) }\DIFaddendFL \\ \hline
\DIFaddbeginFL \DIFaddFL{DRQN-AL }& \DIFaddFL{1/10 }& \DIFaddFL{18.4\% (23/125) }\\ \hline
\DIFaddendFL \end{tabular}
\end{table}

\DIFdelbegin \DIFdel{Table of Task Understanding (Random)~\ref{tab:task-understanding}}\DIFdelend \DIFaddbegin \DIFadd{The results for the task understanding tests are shown in Table~\ref{tab:task-understanding}.  As shown, all the networks show poor performance for task understanding.  The DQN architectures do outperform the DAL architectures, although only by a small margin.
}\DIFaddend \begin{table}[]
\centering
