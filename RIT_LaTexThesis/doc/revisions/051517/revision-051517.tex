...
%DIF ADD Thesis.tex                  Mon May 15 10:39:54 2017
%
% THESIS DESCRIPTION:
...
%DIFAUXCMD
\addtocounter{section}{-1}%DIFAUXCMD
\DIFdel{Robots whose path is controlled by the user are termed as automatic guided vehicles (AGVs)}\DIFdelend \DIFaddbegin \subsubsection{\DIFadd{Deep Apprenticeship Reward Network (DARN)}}
\DIFadd{Once the DAQN is trained, the DARN~\mbox{%DIFAUXCMD
\cite{markovikj2014deep} }%DIFAUXCMD
is used to extract the reward function from the learned expert policy in the DAQN.  The DARN has the same architecture as the DAQN (shown in Fig.~\ref{fig:daqn-dal}). However, it uses the output from the DAQN before the softmax in its loss function.  The input to DARN is a $(s,a,s')$ pairing where $s$ is an 83x83 image, $a$ is the action taken at that state, and $s'$ is the next state after taking action $a$ from state $s$. The loss function for DARN uses an L2-norm and is shown in~\ref{eq:darn-loss}.
}\begin{equation}
...
\DIFadd{Where, $DAQN^{PRESOFT}(s,a)$ are the presoft values of the DAQN with inputs $(s,a)$ and $\max_{a'}DAQN^{PRESOFT}(s',a')$ is the maximum presoft value of the DAQN with inputs $(s',a')$}\DIFaddend . \DIFdelbegin \DIFdel{AGVs follow paths using lasers, cameras, or are physically attached to these paths.
These mobile robots follow predefined routes and so, are heavily used in industries for materials handling.  Apart from transporting loads, these kinds of robots are used in risky environments and explorations, like space, mines and underwater. For guiding some AGVs, lines, magnetic tracts, or wires are installed on warehouse floors. Hence, making use of these robots requires an initial investment. Vision-based AGVs follow a physical line or markers ina room: landmark based navigation.  Combining these ideas a robotic system was created for dynamic path following using vision based control.  }%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{This research platform uses a wireless automatic guided robot that does not require any kind of intrusive modifications to be made to }\DIFdelend \DIFaddbegin \DIFadd{$\gamma$ is the discount factor on how much to weight future states. Therefore, the expanded loss function for the DAQN is shown in~\ref{eq:darn-loss-expanded}.  The overall training architecture for DARN is shown in Fig.~\ref{fig:darn-dal}.
}\begin{equation}
     \DIFadd{\label{eq:darn-loss-expanded}
...
\caption{\DIFaddFL{DAL Architecture~\mbox{%DIFAUXCMD
\cite{markovikj2014deep}}%DIFAUXCMD
.}}
\label{fig:darn-dal}
\end{figure}
\DIFadd{The DARN is trained using random state transitions contained in a separate dataset from the expert data, $D_g$.  This allows the system to generalize to all states, even ones not explored by the expert. 
}

\subsubsection{\DIFadd{Results}}
\DIFadd{Once training on the DARN is complete, the system can now be used to predict next actions given a state. Due to }\DIFaddend the \DIFdelbegin \DIFdel{environment apart from installing an overhead camera. 
The camera above the workspace tracks the robot continuously and is used to help correct it to follow a predefined path. The user is allowed to define the path the robot should follow in a GUI created in MATLAB}\DIFdelend \DIFaddbegin \DIFadd{complexity of Space Invaders, this type of network did not generalize to its game play. However, it performed extremely well on the simple game of Freeway, obtaining a score much higher than other non-deep IRL algorithms~\mbox{%DIFAUXCMD
\cite{markovikj2014deep}}%DIFAUXCMD
}\DIFaddend . 

\DIFdelbegin \DIFdel{Using a live image of the environment from a bird's-eye-view, the robot is tracked in real-time using ArUco fiducial markers.  The live images are fed to a software GUI where the user can freehand draw any path they desire directly on top of the image.  The system then sends commands to the robot to move it along this path by using }\DIFdelend \DIFaddbegin \subsection{\DIFadd{Deep Q-Learning from Demonstrations (DQfD)}}
\DIFadd{Hester et al. undertook the challenge of bringing deep reinforcement learning to real world tasks~\mbox{%DIFAUXCMD
...
\DIFadd{Where $\theta'$ are }\DIFaddend the \DIFdelbegin \DIFdel{camera and ArUco markers as feedback control.
Doing this, the user can draw complicated paths around a room and around obstacles for the robot to navigate.  }%DIFDELCMD < 

%DIFDELCMD < %%%
\section{\DIFdel{System Overview}}
%DIFAUXCMD
\addtocounter{section}{-1}%DIFAUXCMD
\DIFdel{This system consisted of four main sub-components; the Robot, OpenCV, MATLAB, and }\DIFdelend \DIFaddbegin \DIFadd{parameters of the target network, and $\theta$ are the parameters of the current network~\mbox{%DIFAUXCMD
...
\DIFadd{Where $a_E$ is the action the expert takes in state $s$.  $l(s,a_E,a)$ is a margin function where it is 0 when $a=a_E$ and a positive number (usually 1) otherwise.  This will allow for the expert's decisions to be weighted higher.  An L2 regularization term is also used and applied to the weights and biases of }\DIFaddend the \DIFdelbegin \DIFdel{Environment workspace itself. Fig.~\ref{fig:workflow} shows the entire system architecture and work flow using these four main components.
...
%DIFDELCMD < \end{figure}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{network to prevent from over-fitting on small expert datasets.  Therefore, the entire loss function is (\ref{eq:ddqn-dqfd}).
}\begin{equation}
     \DIFadd{\label{eq:ddqn-dqfd}
...
\DIFdelbegin \subsection{\DIFdel{Differential Wheel Drive Robot}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
\DIFdel{Four-wheeled differential drive robotics platforms are commonly used in navigation applications.  The system used in this setup was the 4WD Rover I from Lynxmotion, as shown in }\DIFdelend \DIFaddbegin \chapter{\DIFadd{Dataset}}
\section{\DIFadd{Maze World}}
\DIFadd{Maze World is a variation of Grid World, without pits.  An agent is placed inside a binary maze where white spaces are open and black spaces are walls the agent cannot go through.  The objective of the world is for the agent to navigate through the maze to a goal.  An example of this world is shown in }\DIFaddend Fig.\DIFdelbegin \DIFdel{~\ref{fig:robot}.
}%DIFDELCMD < 
...
%DIFDELCMD < \caption{%%%
\DIFdelFL{4WD Rover I from Lynxmotion}%DIFDELCMD < }
%DIFDELCMD < \label{fig:robot}
%DIFDELCMD < %%%
...
sharp angled turns. To control these motors,pulse width modulation (PWM) signals are given to each motor.  This type of signal is usually at a fixed frequency and is a digital square wave ranging from 0V to }\textit{\DIFdel{$V_{max}$}}%DIFAUXCMD
\DIFdel{.  }\textit{\DIFdel{$V_{max}$}} %DIFAUXCMD
\DIFdel{for the specific application performed was 7.2V.  The signal determines on and off time for the motor based on its duty cycle.  A duty cycle of 50\% will result in the motors being powered at half their capacity.A duty cycle of 10\% will result in the motors being powered at a tenth of their capacity.  A duty cycle of 100\% will result in the motors being powered at full capacity
which looks like a DC signal at 7.2V.  These duty cycles are shown in Fig~\ref{fig:pwm}. To control the duty cycle, an Arduino Uno was used which is }\DIFdelend \DIFaddbegin \DIFadd{An agent is placed in the maze randomly in any white space.  The agent is represented by a smaller dark gray square as shown in Fig.~\ref{fig:maze_with_agent}. The goal of the agent is to make it to }\DIFaddend the \DIFdelbegin \DIFdel{main controller used for the four-wheeled robot.  A value of 0 to 255 was
...
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{goal in the least number of steps.  The agent has four possible actions; Move Right, Move Down, Move Left, Move Up. Actions are one-hot enconded. If the agent hits a wall (black) it loses, if it hits the goal it wins. At each step, the agent moves a full grid step.  In the back-end, the maze is treated like a $m x n$ grid, and Python's PIL framework is used to generate the graphics used for simulation and dataset generation.
}\begin{figure}
\DIFaddendFL \centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics[scale=1]{images/PWM.PNG}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[scale=1.5]{images/maze_with_agent.jpg}
\DIFaddendFL \caption{\DIFdelbeginFL \DIFdelFL{Various duty cycles on PWM signals}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Maze World with Agent: small, dark gray square represents agent.}\DIFaddendFL }
\DIFdelbeginFL %DIFDELCMD < \label{fig:pwm}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \label{fig:maze_with_agent}
\DIFaddendFL \end{figure}

\DIFdelbegin \DIFdel{This robot was programmed through the Arduino to take in serial commands to determine motion.  Single character commands were used as shown in Table~\ref{tab:movement}. To take in these serial commands, a Bluetooth module was connected to the Arduino's TX and RX lines.  The module used was the HC-06, which is a low-power slave Bluetooth module.  Once the Arduino receives a movement command it sends the appropriate PWM signals to the Pololu Dual MC3926 motor controller board which was wired
...
\DIFdelFL{Duty cycle of all wheels set to 0\%, no movement }%DIFDELCMD < \\ \hline
%DIFDELCMD < \end{tabular}
%DIFDELCMD < \end{table}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \subsection{\DIFadd{Expert Data}}
...
\subsection{\DIFdelbegin \DIFdel{OpenCV}\DIFdelend \DIFaddbegin \DIFadd{Random Data}\DIFaddend }
\DIFaddbegin \label{sec:random_data}
...
\DIFdelbegin \DIFdel{OpenCV is a comprehensive computer vision toolbox used for a variety of real-time applications such as tracking, facial recognition, segmentation, and classification. It is originally written in C++ but wrappers have been made to allow it to be used in Python and MATLAB. For thissystem, the only component of OpenCV being used is video stream capture and }\DIFdelend \DIFaddbegin \subsection{\DIFadd{Datasets}}
\DIFadd{Expert data and random data are stored as GIFs.  To do this, at each movement of }\DIFaddend the \DIFdelbegin \DIFdel{ArUco Marker detection.
ArUco markers are pattern-based black and white squares that can be easily identified through OpenCV.  Since OpenCV knows what patterns to look for, it can identify ArUco markers in an environment and determine the corners of them as well as the orientation. For this application four ArUco markers were used as shown }\DIFdelend \DIFaddbegin \DIFadd{agent an image is generated (such as }\DIFaddend in Fig.~\DIFdelbegin \DIFdel{\ref{fig:ArUco}. This marker array was printed to the size 14.5 cm by 14.5 cm and was  mounted flat on top of the robot so that the center of the array was at the center of the robot. The top two markers were facing in the direction of the front of the robot and were used as a reference to determine the orientation of the robot.  }\DIFdelend \DIFaddbegin \DIFadd{\ref{fig:maze_with_agent}) to represent the current state of the agent in the maze.  For expert data these images are collected until the agent reaches the goal. The images are then combined into a GIF and saved for later use.  Each action the agent takes is saved into a text file to be used later. The same is done for the random data, except that the GIFs were only two images total.
}\DIFaddend 

\DIFdelbegin %DIFDELCMD < \begin{figure}[h!]
%DIFDELCMD < \centering
%DIFDELCMD < \includegraphics[scale=.45]{images/ArucoArray.PNG}
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
...
%DIFDELCMD < \end{figure}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{Two types of datasets were created for expert data and random data; same and random.
}\subsubsection{\DIFadd{Same}}
\DIFadd{"Same" means that the maze the agent is in does not change.  Therefore, the expert data and random data use the same maze, however the placement of the agent will change.  This dataset is used to show whether or not a system can learn a single task without generalizing what the task is.
}\DIFaddend 
...
%DIFDELCMD < %%%
\DIFdel{MATLAB is a powerful computing platform.  It uses a scripting based language which isideal for fast prototyping.
...
%DIFDELCMD < %%%
\DIFdel{The environment that the robot resides in can be variable.
However, }\DIFdelend \DIFaddbegin \subsubsection{\DIFadd{Random}}
\DIFadd{"Random" means that the maze the agent is in does change.  Therefore, }\DIFaddend the \DIFdelbegin \DIFdel{surface on which the robot roams needs to be navigable by the robot chassis. Also, }\DIFdelend \DIFaddbegin \DIFadd{expert data and random data use different randomly generated mazes, and }\DIFaddend the \DIFdelbegin \DIFdel{environment needs to be well lit so that the markers can be detected.  A webcam was placed on the ceiling of the room which was about 8 feet from the ground and faced perpendicular to the floor.  The webcam used is shown in Fig.~\ref{fig:webcam}. The webcam itself was used with a resolution of 640 px by 480 px.  This created view-able workspace of about 1.8m (6 feet) by 1.4m (4.5 feet).  The robot was placed within this static workspace. The webcam was connected to the computer running the MATLAB application through USB}\DIFdelend \DIFaddbegin \DIFadd{placement of the agent changes in each maze.  This dataset is used to show whether or not a system can learn a single task while also generalizing what the task is.
}

\subsection{\DIFadd{Simulation}}
\DIFadd{Given a maze, current placement of the agent, and the action to be performed, a simulation of the maze task can be completed.  This is used in testing as well as in training to show how well the agent is learning and to gather data.  At each step the simulation environment tells the system the new state of the agent (image of maze with agent), whether or not the agent has reached the goal, and whether or not the agent has crashed into a wall.  If the simulation is fed an initial maze image, it will keep that maze for the rest of the simulation (in cases where the same maze is being tested) otherwise it will use a random generated maze.  If the simulation is given no initial state, it will randomly place the agent in the maze}\DIFaddend .

\DIFdelbegin %DIFDELCMD < \begin{figure}[h!]
%DIFDELCMD < \centering
%DIFDELCMD < \includegraphics[scale=.15]{images/webcam.png}
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
...
%DIFDELCMD < %%%
\section{\DIFdel{Procedure}}
%DIFAUXCMD
\addtocounter{section}{-1}%DIFAUXCMD
\DIFdel{As an overview, the procedure of running this system takes into account all four main components}\DIFdelend \DIFaddbegin \subsection{\DIFadd{Processing Data}}
\DIFadd{To make the saved GIFs and action-list text files usable to the system architectures, some preprocessing needs to be done}\DIFaddend .  First, the \DIFdelbegin \DIFdel{camera is set-up to view a bird's-eye view of the workspace.  The robot is then placed within that workspace with its ArUco markers.  The application is then started using MATLAB.  OpenCV functions are used to detect the ArUco markers and determine the millimeter to pixel ratio since the ArUco markers are a known size.  Once the robot is detected the user is prompted to draw a path over the current image . This path is then fed to the path planner code where the robot iteratively follows the following steps:
...
\DIFdel{Once the point is reached, }\DIFdelend \DIFaddbegin \DIFadd{GIFs are read in frame by frame.  These frames are then converted to }\DIFaddend the \DIFdelbegin \DIFdel{desired point moves to the next point
}%DIFDELCMD < \end{itemize}
%DIFDELCMD < %%%
\DIFdel{Once the robot completes the desired path the errors from the robot movement versus the ground truth path are found.  The user is then prompted to draw another path.  This basic procedure is shown in the flowchart in Fig~\ref{fig:flowchart}.
}\DIFdelend \DIFaddbegin \DIFadd{data size needed using PIL image resize function.  These are then saved into a numpy array of shape $[?,data size, data size, 1]$.  As shown, all the data is square.  The actions are read out of the action-list and converted into one-hot encoded vectors stored into a numpy array of shape $[?,number of actions]$.  These arrays are then split into test, train, and reward datasets for the image data and action data.  These split arrays are then stored into an h5py file so that it can be accessed faster and more efficiently when the system is run again. 
}\DIFaddend 
...
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \chapter{\DIFadd{Proposed Architectures}}
...
}\DIFaddend \subsection{\DIFdelbegin \DIFdel{Detecting ArUco Markers}\DIFdelend \DIFaddbegin \DIFadd{Transfer Learning}\DIFaddend }
\DIFdelbegin %DIFDELCMD < 
...
\DIFdel{which takes in an image (thatcontains the markers), a dictionary of marker definitions, and some optional parameters. It then returns the corner locations and ID numbers for all markers detected.  Once the corners and IDs are found, as shown in Fig.  ~\ref{fig:ArUcodetected}, }\DIFdelend \DIFaddbegin \DIFadd{Another change to the DAL architecture is the use of transfer learning.  In the current implementation of DAL, the DAQN and DARN are trained seperatley.  This means that, while they use the same network architecture, }\DIFaddend the \DIFdelbegin \DIFdel{center point for the array is also found.  }%DIFDELCMD < \begin{figure}[h!]
%DIFDELCMD < \centering
...
%DIFDELCMD < \end{figure}
%DIFDELCMD <  %%%
\DIFdel{Once the user draws the path the path is represented by a solid red line with a white star at its first point and a yellow star at its end point.  }\DIFdelend \DIFaddbegin \DIFadd{~\ref{eq:dal-dqn-loss}
}\DIFaddend 
...
%DIFDELCMD < %%%
\DIFdel{To ensure the robot follows the path accurately, thresholds were created for }\DIFdelend \DIFaddbegin \begin{equation}
...
\DIFadd{An inherit issue with this representation is the need for the reward $R$.  The purpose of apprenticeship learning is to find the reward model from given expert demonstrations, which means that the reward for each step an agent or expert takes is not known. By abstracting the reward function from step-level to task-level, it serves as a task completion modifier for the loss.  For example, for each step taken by the expert if the task is not complete the reward is $0$, if the task is complete the reward is $+1$, and if the task is failed the reward is $-1$.  Since the expert data used is all successful, only the last transition for the expert into the goal is given a reward of $+1$.  For the DARN network, }\DIFaddend the \DIFdelbegin \DIFdel{allowable distance it was from the desired point and its orientation with respect to the next point.  To identify the distance in pixels between the desired point and the robot, denoted as }\textit{\DIFdel{d}}%DIFAUXCMD
\DIFdel{, the euclidean distance between the desired point and the center point of the ArUco marker array was found.  This equation is shown in \ref{eqn:distance}, where $x_c$ and $y_c$ are the pixel coordinates for the center of the robot, and $x_g$ and $y_g$ are the pixel coordinates for the desired point (current point on path trying to be reached) . }\DIFdelend \DIFaddbegin \DIFadd{same loss function is used, but instead of using random data (\ref{sec:random_data}) the network "plays against itself" to accrue more experience.  Therefore, the agent starts in a maze in a random location and performs a step based on the network output from the state it is in.  }\DIFaddend If the \DIFdelbegin \DIFdel{value of }\textit{\DIFdel{d}} %DIFAUXCMD
\DIFdel{was less than the threshold (by default was 20 pixels) then the robot met the distance criteria.  }\DIFdelend \DIFaddbegin \DIFadd{step is to the goal the reward is $+1$ if it hits a wall it is $-1$ (and the maze resets), otherwise it is $0$.  
}\DIFaddend 
...
\DIFdelbegin \DIFdel{The second criteria was the orientation of the robot to the next point.  
This was important to meet so that the robot was always following the trajectory of }\DIFdelend \DIFaddbegin \section{\DIFadd{Deep Q-Networks Implementations}}
...
\cite{atari} }%DIFAUXCMD
also introduced the concept of using a separate target Q-network.  This was due to the fact that }\DIFaddend the \DIFdelbegin \DIFdel{path.  To do this 3 vectors were found and plotted. The first vector was from the center of the robot to the desired point ($V_g$).  The second vector was from the center of the robot to the front of the robot between ArUco markers 31 and 203 ($V_o$).  The third vector was from the front of the robot to the desired point ($V_n$).
These vectors are shown in Fig.  ~\ref{fig:orientation}.
}%DIFDELCMD < 
...
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{allowed for a variety of debugging strategies.  Much of the research in deep learning or reinforcement learning is done using Python, so running example code from others' research was made easy by using this scripting language.
}\subsection{\DIFadd{TensorFlow}}
\DIFadd{TensorFlow 1.0~\mbox{%DIFAUXCMD
\cite{tensorflow2015-whitepaper} }%DIFAUXCMD
was used for this research.  TensorFlow is an open source library for Python developed by Google.  It is mostly used for numerical computation using data flow graphs, and is primarily used to build a variety of neural networks.  This framework allows users to easily implement deep architectures and run a variety of analysis and training tool.  
}\DIFaddend \subsection{\DIFdelbegin \DIFdel{Following the Path}\DIFdelend \DIFaddbegin \DIFadd{Python Imaging Library (PIL)}\DIFaddend }
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Using Algorithm~\ref{alg:movement}, the robot went to each point on the path drawn by the user. As it did this , the center point of the robot was plotted every time a point was reached.
Using this data the, the overall error of the physical robot path was determined.  Fig.  ~\ref{fig:pathfollowing} shows a snapshot of a robot following a path. 
}%DIFDELCMD < 
...
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{Python Imaging Library (PIL) adds image processing and creation tools to Python. The PIL library was used in many ways in this research including dataset generation, dataset processing, and resut visualization.
}\subsection{\DIFadd{Numpy}}
\DIFadd{Numpy allows fro complex data management and matematical operations, especially in the linear algebra space.  TensorFlow heavily relies on numpy for its powerful matematical operations.  Numpy was also used in this research for data storage and manipulation. 
}\DIFaddend \subsection{\DIFdelbegin \DIFdel{Accuracy Analysis}\DIFdelend \DIFaddbegin \DIFadd{h5py}\DIFaddend }
\DIFdelbegin \DIFdel{To determine the overall accuracy of the robot, error between the physical path the robot took and the actual path was measured.  This accuracy was improved by changing the distance and orientation thresholds for the system. At the end of each path taken, the error magnitude for each point is displayed as well as the standard deviation of error, mean error, maximum error, minimum error, and estimated error. The estimated error is defined as the error that will be found 95\% of the time or two standard deviations ($\sigma$) away from the mean ($\mu$), as shown in Equation~\ref{eqn:esterror}}\DIFdelend \DIFaddbegin \DIFadd{h5py is a Python package to implement the HDF5 binary data format.  This format allows for the storage of large datasets in a compressed way.  In this research, the datasets were often large and a huge bottleneck for the system, however, after converting the data and storing it with h5py, reading the data was done in seconds.  This severly cut-down on runtime (from 6 hours for data reading to 30 seconds)}\DIFaddend . 
...
%DIFAUXCMD
\DIFdelend \DIFaddbegin \section{\DIFadd{Architecture Details}}
\DIFadd{A number of architectures were created and tested as described in~\ref{sec:proposed_architectures}.  They hyper-parameters for each of these architectures were tuned to ensure best results for the networks.  These hyper-parameters include:
}\begin{itemize}
  \item \DIFadd{Dataset Split: Number of training data, number of testing data, and number of random data
  }\item \DIFadd{Data size: Size of input data
  }\item \DIFadd{Pooling: Boolean whether or not to pool after convolution layers
  }\item \DIFadd{Epochs: Number of epochs for training from expert, number of epochs for training agent
  }\item \DIFadd{Learning Rate: Rate at which network weights update
  }\item \DIFadd{Discount Factor ($\gamma$): Weight of future state values
  }\item \DIFadd{Reward Value: Arbitrary value for maximum and minimum award
  }\item \DIFadd{Shared Experience Ratio: Percentage of expert experience used when training agent
  }\item \DIFadd{Replay Buffer: Size of experience memory
}\end{itemize}
\DIFaddend 


\DIFdelbegin \DIFdel{Three trials were tested with varying threshold values.  Fig.~\ref{fig:results1},~\ref{fig:results2}, and~\ref{fig:results3} show several paths with their corresponding error charts.  }%DIFDELCMD < 

%DIFDELCMD < \begin{figure}[h!]
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \chapter{\DIFaddFL{Results and Analysis}}
\section{\DIFaddFL{Task Completion versus Task Understanding}}
\section{\DIFaddFL{Reading the Data}}
\DIFaddFL{Test success and action prediction
}\section{\DIFaddFL{Proposed Architecture Performances}}
\DIFaddFL{Table of Task Completion (Same)~\ref{tab:task-complete}}
\DIFaddendFL \centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics[scale=.25]{images/PATH-2095-1.PNG}
...
%DIFDELCMD < \begin{table}[h!]
%DIFDELCMD < %%%
\DIFdelbeginFL %DIFDELCMD < \label{tab:error}
%DIFDELCMD < \begin{tabular}{ | p{0.5cm} | p{0.5cm} | p{0.5cm} | p{0.75cm} | p{0.75cm} | p{0.75cm} | p{1cm} | }
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \label{tab:task-complete}
...
}\DIFdelend \DIFaddbegin \section{\DIFadd{Result Analysis}}
\DIFadd{Include loss graphs
}\DIFaddend 
